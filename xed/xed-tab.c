	...
	switch (prop_id)
	{
		...
		case PROP_AUTO_SAVE_INTERVAL:
			g_value_set_int (value,
					 xed_tab_get_auto_save_interval (tab));
			break;
		default:
		        ...
		        GParamSpec   *pspec)
{
	...
	{
		case PROP_AUTO_SAVE:
			xed_tab_set_auto_save_enabled (tab,
							 g_value_get_boolean (value));
			break;
		case PROP_AUTO_SAVE_INTERVAL:
			xed_tab_set_auto_save_interval (tab,
...
show_loading_message_area (XedTab *tab)
{
	...
	else
	{
		...
		area = xed_progress_message_area_new (GTK_STOCK_OPEN,
							msg,
							TRUE);
	}
...
show_saving_message_area (XedTab *tab)
{
	...
	area = xed_progress_message_area_new (GTK_STOCK_SAVE,
						msg,
						FALSE);
		 ...
		 XedTab      *tab)
{
	...
	if (error != NULL &&
	    (error->domain != XED_DOCUMENT_ERROR || error->code != XED_DOCUMENT_ERROR_CONVERSION_FALLBACK))
	{
		...
		else
		{
			...
			{
				emsg = xed_io_loading_error_message_area_new (uri,
										tab->priv->tmp_encoding,
										error);
	...
	else
	{
		...
		_xed_recent_add (XED_WINDOW (gtk_widget_get_toplevel (GTK_WIDGET (tab))),
				   uri,
				   mime);
		g_free (mime);
		...
		XedTab      *tab)
{
	...
	if (error != NULL)
	{
		...
		else
		{
			...
			emsg = xed_conversion_error_while_saving_message_area_new (
									tab->priv->tmp_save_uri,
									tab->priv->tmp_encoding,
									error);
...
xed_tab_init (XedTab *tab)
{
	...
	_xed_document_set_mount_operation_factory (doc,
...
{
	return XED_DOCUMENT (gtk_text_view_get_buffer (
		      ...
		      XedPrintJob *job)
{
	...
	_xed_app_set_default_print_settings (xed_app_get_default (),
					       settings);
	...
	/* make it the default */
	_xed_app_set_default_page_setup (xed_app_get_default (),
					   page_setup);
}
...
static void
show_printing_message_area (XedTab *tab, gboolean preview)
{
	GtkWidget *area;

	if (preview)
		area = xed_progress_message_area_new (GTK_STOCK_PRINT_PREVIEW,
							"",
							TRUE);
	else
		area = xed_progress_message_area_new (GTK_STOCK_PRINT,
							"",
							TRUE);
...
_xed_tab_print (XedTab     *tab)
{
	...
	xed_tab_print_or_print_preview (tab,
