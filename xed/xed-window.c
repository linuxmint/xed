...
static void
xed_window_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
...
static gboolean
xed_window_window_state_event (GtkWidget *widget, GdkEventWindowState *event)
{
...
static gboolean
xed_window_configure_event (GtkWidget *widget, GdkEventConfigure *event)
{
    XedWindow *window = XED_WINDOW (widget);
 ...
 */
static gboolean
xed_window_key_press_event (GtkWidget *widget, GdkEventKey *event)
{
...
static void
xed_window_tab_removed (XedWindow *window, XedTab *tab)
{
    xed_plugins_engine_garbage_collect (xed_plugins_engine_get_default ());
}
...
xed_window_class_init (XedWindowClass *klass)
{
    ...
    signals[TAB_ADDED] = g_signal_new ("tab_added", G_OBJECT_CLASS_TYPE (object_class),
            G_SIGNAL_RUN_FIRST, G_STRUCT_OFFSET (XedWindowClass, tab_added), NULL, NULL,
            g_cclosure_marshal_VOID__OBJECT, G_TYPE_NONE, 1, XED_TYPE_TAB);

    signals[TAB_REMOVED] = g_signal_new ("tab_removed", G_OBJECT_CLASS_TYPE (object_class),
            G_SIGNAL_RUN_FIRST, G_STRUCT_OFFSET (XedWindowClass, tab_removed), NULL, NULL,
            g_cclosure_marshal_VOID__OBJECT, G_TYPE_NONE, 1, XED_TYPE_TAB);

    signals[TABS_REORDERED] = g_signal_new ("tabs_reordered", G_OBJECT_CLASS_TYPE (object_class),
            G_SIGNAL_RUN_FIRST, G_STRUCT_OFFSET (XedWindowClass, tabs_reordered), NULL, NULL,
            g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

    signals[ACTIVE_TAB_CHANGED] = g_signal_new ("active_tab_changed", G_OBJECT_CLASS_TYPE (object_class),
            G_SIGNAL_RUN_FIRST, G_STRUCT_OFFSET (XedWindowClass, active_tab_changed), NULL, NULL,
            g_cclosure_marshal_VOID__OBJECT, G_TYPE_NONE, 1, XED_TYPE_TAB);

    signals[ACTIVE_TAB_STATE_CHANGED] = g_signal_new ("active_tab_state_changed", G_OBJECT_CLASS_TYPE (object_class),
            G_SIGNAL_RUN_FIRST, G_STRUCT_OFFSET (XedWindowClass, active_tab_state_changed), NULL, NULL,
            g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

    g_object_class_install_property (object_class, PROP_STATE,
            g_param_spec_flags ("state", "State", "The window's state",
                    XED_TYPE_WINDOW_STATE, XED_WINDOW_STATE_NORMAL, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));

    g_type_class_add_private (object_class, sizeof(XedWindowPrivate));
}

static void
menu_item_select_cb (GtkMenuItem *proxy, XedWindow *window)
{
    GtkAction *action;
    char *message;
...
static void
menu_item_deselect_cb (GtkMenuItem *proxy, XedWindow *window)
{
    gtk_statusbar_pop (GTK_STATUSBAR (window->priv->statusbar), window->priv->tip_message_cid);
}

static void
connect_proxy_cb (GtkUIManager *manager, GtkAction *action, GtkWidget *proxy, XedWindow *window)
{
...
static void
disconnect_proxy_cb (GtkUIManager *manager, GtkAction *action, GtkWidget *proxy, XedWindow *window)
{
    if (GTK_IS_MENU_ITEM (proxy)) {
...
static void
apply_toolbar_style (XedWindow *window, GtkWidget *toolbar)
{
...
static gboolean
set_toolbar_style (XedWindow *window, XedWindow *origin)
{
    gboolean visible;
...
static void
update_next_prev_doc_sensitivity (XedWindow *window, XedTab    *tab)
{
    gint         tab_number;
...
static void
received_clipboard_contents (GtkClipboard *clipboard, GtkSelectionData *selection_data, XedWindow *window)
{
    gboolean sens;
    GtkAction *action;
...
static void
set_paste_sensitivity_according_to_clipboard (XedWindow *window, GtkClipboard *clipboard)
{
    GdkDisplay *display;
...
static void
set_sensitivity_according_to_tab (XedWindow *window, XedTab *tab)
{
    XedDocument  *doc;
    XedView      *view;
    GtkAction    *action;
    ...
    gtk_action_set_sensitive (action,
        (state_normal || (state == XED_TAB_STATE_SHOWING_PRINT_PREVIEW)));
    ...
    gtk_action_set_sensitive (action,
                  (state_normal || state == XED_TAB_STATE_EXTERNALLY_MODIFIED_NOTIFICATION) && b);
...
static void
language_toggled (GtkToggleAction *action, XedWindow *window)
{
    XedDocument *doc;
...
static void
create_language_menu_item (GtkSourceLanguage *lang, gint index, guint ui_id, XedWindow *window)
{
    ...
    if (section_action == NULL) {
        ...
        section_action = gtk_action_new (escaped_section, section_name, NULL, NULL);
        ...
        gtk_ui_manager_add_ui (window->priv->manager, ui_id,
                ...
                escaped_section, escaped_section, GTK_UI_MANAGER_MENU, FALSE);
    }
...
void
_xed_recent_add (XedWindow *window, const gchar *uri, const gchar *mime)
{
    GtkRecentManager *recent_manager;
...
void
_xed_recent_remove (XedWindow *window, const gchar *uri)
{
    GtkRecentManager *recent_manager;
...
static void
open_recent_file (const gchar *uri, XedWindow *window)
{
    GSList *uris = NULL;
...
static void
recent_chooser_item_activated (GtkRecentChooser *chooser, XedWindow  *window)
{
    gchar *uri;
...
static void
recents_menu_activate (GtkAction *action, XedWindow *window)
{
    GtkRecentInfo *info;
    const gchar *uri;
...
static void
recent_manager_changed (GtkRecentManager *manager, XedWindow      *window)
{
...
update_recent_files_menu (XedWindow *window)
{
    ...
    for (l = filtered_items; l != NULL; l = l->next) {
        ...
        g_object_set_data_full (G_OBJECT (action),
                "gtk-recent-info", gtk_recent_info_ref (info), (GDestroyNotify) gtk_recent_info_unref);
        ...
        gtk_ui_manager_add_ui (p->manager, p->recents_menu_ui_id,
...
static void
toolbar_visibility_changed (GtkWidget *toolbar, XedWindow *window)
{
    gboolean visible;
    GtkAction *action;
...
static GtkWidget *
setup_toolbar_open_button (XedWindow *window, GtkWidget *toolbar)
{
    GtkRecentManager *recent_manager;
...
static void
create_menu_bar_and_toolbar (XedWindow *window, GtkWidget   *main_box)
{
    ...
    recent_manager = gtk_recent_manager_get_default ();
    window->priv->recents_handler_id = g_signal_connect (recent_manager, "changed",
            G_CALLBACK (recent_manager_changed), window);
    update_recent_files_menu (window);
...
static void
documents_list_menu_activate (GtkToggleAction *action, XedWindow     *window)
{
    gint n;
    if (gtk_toggle_action_get_active (action) == FALSE) {
        return;
...
update_documents_list_menu (XedWindow *window)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        gtk_ui_manager_add_ui (p->manager, id, "/MenuBar/DocumentsMenu/DocumentsListPlaceholder",
                action_name, action_name, GTK_UI_MANAGER_MENUITEM, FALSE);
...
static gboolean
set_statusbar_style (XedWindow *window, XedWindow *origin)
{
    GtkAction *action;
    gboolean visible;
    if (origin == NULL) {
...
static void
statusbar_visibility_changed (GtkWidget   *statusbar, XedWindow *window)
{
    gboolean visible;
    GtkAction *action;
...
static void
tab_width_combo_changed (XedStatusComboBox *combo, GtkMenuItem *item, XedWindow *window)
{
    XedView *view;
    guint width_data = 0;
...
static void
use_spaces_toggled (GtkCheckMenuItem *item, XedWindow *window)
{
    XedView *view;
...
static void
language_combo_changed (XedStatusComboBox *combo, GtkMenuItem *item, XedWindow *window)
{
    XedDocument *doc;
...
fill_language_combo (XedWindow *window)
{
    ...
    xed_status_combo_box_add_item (
            XED_STATUS_COMBO_BOX (window->priv->language_combo), GTK_MENU_ITEM (menu_item), name);
    ...
    for (item = languages; item; item = item->next) {
        ...
        g_object_set_data_full (
                G_OBJECT (menu_item), LANGUAGE_DATA, g_object_ref (lang), (GDestroyNotify)g_object_unref);
        xed_status_combo_box_add_item (
                XED_STATUS_COMBO_BOX (window->priv->language_combo), GTK_MENU_ITEM (menu_item), name);
    }
          ...
          GtkWidget   *main_box)
{
    ...
    window->priv->generic_message_cid =
            gtk_statusbar_get_context_id (GTK_STATUSBAR (window->priv->statusbar), "generic_message");
    window->priv->tip_message_cid =
            gtk_statusbar_get_context_id(GTK_STATUSBAR (window->priv->statusbar), "tip_message");
...
static void
update_cursor_position_statusbar (GtkTextBuffer *buffer, XedWindow   *window)
{
    gint row, col;
    GtkTextIter iter;
    GtkTextIter start;
...
static void
update_overwrite_mode_statusbar (GtkTextView *view, XedWindow *window)
{
...
static void
set_tab_width_item_blocked (XedWindow *window, GtkMenuItem *item)
{
...
static void
spaces_instead_of_tabs_changed (GObject *object, GParamSpec  *pspec, XedWindow *window)
{
...
static void
tab_width_changed (GObject *object, GParamSpec  *pspec, XedWindow *window)
{
    GList *items;
    GList *item;
    XedStatusComboBox *combo = XED_STATUS_COMBO_BOX (window->priv->tab_width_combo);
...
static void
language_changed (GObject     *object, GParamSpec  *pspec, XedWindow *window)
{
    GList *items;
    GList *item;
    ...
    for (item = items; item; item = item->next) {
        ...
        if ((new_id == NULL && lang == NULL) ||
            ...
            xed_status_combo_box_set_item (
                    XED_STATUS_COMBO_BOX (window->priv->language_combo), GTK_MENU_ITEM (item->data));
...
static void
notebook_switch_page (GtkNotebook *book, GtkWidget *pg, gint page_num, XedWindow *window)
{
    XedView *view;
    XedTab *tab;
    GtkAction *action;
    ...
    window->priv->tab_width_id = g_signal_connect (view, "notify::tab-width",
...
static void
update_tab_autosave (GtkWidget *widget, gpointer   data)
{
...
static void
analyze_tab_state (XedTab *tab, XedWindow *window)
{
    XedTabState ts;
...
update_window_state (XedWindow *window)
{
    ...
    if (old_ws != window->priv->state) {
        ...
        xed_statusbar_set_window_state (XED_STATUSBAR (window->priv->statusbar),
                window->priv->state, window->priv->num_tabs_with_error);
...
static void
sync_state (XedTab *tab, GParamSpec *pspec, XedWindow *window)
{
    xed_debug (DEBUG_WINDOW);
...
static void
sync_name (XedTab    *tab, GParamSpec  *pspec, XedWindow *window)
{
    GtkAction *action;
    gchar *action_name;
    gchar *tab_name;
...
static void
load_uris_from_drop (XedWindow  *window, gchar **uri_list)
{
    GSList *uris = NULL;
    gint i;
...
static void
drag_data_received_cb (GtkWidget *widget, GdkDragContext *context, gint x, gint y, GtkSelectionData *selection_data,
...
static void
drop_uris_cb (GtkWidget *widget, gchar **uri_list)
{
    XedWindow *window;
    window = get_drop_window (widget);
    if (window == NULL) {
...
fullscreen_controls_show (XedWindow *window)
{
    ...
    gdk_screen_get_monitor_geometry (screen,
            gdk_screen_get_monitor_at_window (screen, gtk_widget_get_window (GTK_WIDGET (window))), &fs_rect);
...
static void
show_hide_fullscreen_toolbar (XedWindow *window, gboolean show, gint height)
{
    ...
    if (enable_animations) {
        ...
        if (window->priv->fullscreen_animation_timeout_id == 0) {
            window->priv->fullscreen_animation_timeout_id =
                    g_timeout_add (FULLSCREEN_ANIMATION_SPEED, (GSourceFunc) run_fullscreen_animation, window);
        }
    }
    else {
        GdkRectangle fs_rect;
...
static gboolean
on_fullscreen_controls_enter_notify_event (GtkWidget *widget, GdkEventCrossing *event, XedWindow *window)
{
...
static gboolean
on_fullscreen_controls_leave_notify_event (GtkWidget *widget, GdkEventCrossing *event, XedWindow *window)
{
    GdkDisplay *display;
    GdkScreen *screen;
    gint w, h;
...
static void
can_search_again (XedDocument *doc, GParamSpec *pspec, XedWindow *window)
{
    gboolean sensitive;
    GtkAction *action;
...
static void
sync_languages_menu (XedDocument *doc, GParamSpec *pspec, XedWindow *window)
{
    update_languages_menu (window);
    xed_plugins_engine_update_plugins_ui (xed_plugins_engine_get_default (), window);
}
...
static void
readonly_changed (XedDocument *doc, GParamSpec  *pspec, XedWindow *window)
{
...
static void
editable_changed (XedView  *view, GParamSpec  *arg1, XedWindow *window)
{
    xed_plugins_engine_update_plugins_ui (xed_plugins_engine_get_default (), window);
}
...
static void
notebook_tab_added (XedNotebook *notebook, XedTab *tab, XedWindow *window)
{
    XedView *view;
    XedDocument *doc;
...
static void
notebook_tab_removed (XedNotebook *notebook, XedTab *tab, XedWindow *window)
{
    XedView     *view;
    XedDocument *doc;
...
static void
notebook_tabs_reordered (XedNotebook *notebook, XedWindow *window)
{
    update_documents_list_menu (window);
    update_next_prev_doc_sensitivity_per_window (window);
...
static void
notebook_tab_detached (XedNotebook *notebook, XedTab *tab, XedWindow *window)
{
...
static void
notebook_tab_close_request (XedNotebook *notebook, XedTab *tab, GtkWindow *window)
{
...
static gboolean
show_notebook_popup_menu (GtkNotebook *notebook, XedWindow *window, GdkEventButton *event)
{
    GtkWidget *menu;
//  GtkAction *action;
...
// CHECK do we need this?
    ...
    if (event != NULL) {
        ...
        gtk_menu_popup (GTK_MENU (menu), NULL, NULL,
                xed_utils_menu_position_under_widget, tab_label, 0, gtk_get_current_event_time ());
...
static gboolean
notebook_button_press_event (GtkNotebook *notebook, GdkEventButton *event, XedWindow *window)
{
...
static gboolean
notebook_popup_menu (GtkNotebook *notebook, XedWindow *window)
{
...
static void
side_panel_size_allocate (GtkWidget *widget, GtkAllocation *allocation, XedWindow   *window)
{
    window->priv->side_panel_size = allocation->width;
}

static void
bottom_panel_size_allocate (GtkWidget *widget, GtkAllocation *allocation, XedWindow   *window)
{
    window->priv->bottom_panel_size = allocation->height;
}

static void
hpaned_restore_position (GtkWidget *widget, XedWindow *window)
{
    gint pos;
...
static void
vpaned_restore_position (GtkWidget *widget, XedWindow *window)
{
    ...
    xed_debug_message (DEBUG_WINDOW,
            "Restoring vpaned position: bottom panel size %d", window->priv->bottom_panel_size);
...
static void
side_panel_visibility_changed (GtkWidget *side_panel, XedWindow *window)
{
    gboolean visible;
    GtkAction *action;
...
create_side_panel (XedWindow *window)
{
    ...
    xed_panel_add_item_with_stock_icon (XED_PANEL (window->priv->side_panel),
            documents_panel, _("Documents"), GTK_STOCK_FILE);
}

static void
bottom_panel_visibility_changed (XedPanel *bottom_panel, XedWindow *window)
{
    gboolean visible;
    GtkAction *action;
...
static void
bottom_panel_item_removed (XedPanel *panel, GtkWidget *item, XedWindow *window)
{
...
static void
bottom_panel_item_added (XedPanel *panel, GtkWidget *item, XedWindow *window)
{
...
init_panels_visibility (XedWindow *window)
{
    ...
    g_signal_connect (window->priv->bottom_panel, "item_added",
            G_CALLBACK (bottom_panel_item_added), window);
}

static void
clipboard_owner_change (GtkClipboard *clipboard, GdkEventOwnerChange *event, XedWindow *window)
{
    set_paste_sensitivity_according_to_clipboard (window, clipboard);
}

static void
window_realized (GtkWidget *window, gpointer *data)
{
    GtkClipboard *clipboard;
...
static void
window_unrealized (GtkWidget *window, gpointer  *data)
{
    GtkClipboard *clipboard;
    clipboard = gtk_widget_get_clipboard (window, GDK_SELECTION_CLIPBOARD);
...
static void
check_window_is_active (XedWindow *window, GParamSpec *property, gpointer useless)
{
...
static void
connect_notebook_signals (XedWindow *window, GtkWidget   *notebook)
{
...
static void
add_notebook (XedWindow *window, GtkWidget   *notebook)
{
 ...
 */
XedTab *
xed_window_create_tab (XedWindow *window, gboolean jump_to)
{
    XedTab *tab;
 ...
 */
XedTab *
xed_window_create_tab_from_uri (XedWindow *window, const gchar *uri, const XedEncoding *encoding, gint line_pos,
        gboolean create, gboolean jump_to)
{
    GtkWidget *tab;
 ...
 */
void
xed_window_close_tab (XedWindow *window, XedTab *tab)
{
 ...
 */
void
xed_window_close_tabs (XedWindow *window, const GList *tabs)
{
    g_return_if_fail (XED_IS_WINDOW (window));
    g_return_if_fail (!(window->priv->state & XED_WINDOW_STATE_SAVING) &&
            !(window->priv->state & XED_WINDOW_STATE_SAVING_SESSION));
...
XedWindow *
_xed_window_move_tab_to_new_window (XedWindow *window, XedTab *tab)
{
    XedWindow *new_window;
 ...
 */
void
xed_window_set_active_tab (XedWindow *window, XedTab *tab)
{
    gint page_num;
...
void
_xed_window_set_default_location (XedWindow *window, GFile *location)
{
    GFile *dir;
...
void
_xed_window_set_saving_session_state (XedWindow *window, gboolean saving_session)
{
...
static void
hide_notebook_tabs_on_fullscreen (GtkNotebook *notebook, GParamSpec *pspec, XedWindow *window)
{
 ...
 */
XedTab *
xed_window_get_tab_from_location (XedWindow *window, GFile *location)
{
    GList *tabs;
    GList *l;
    XedTab *ret = NULL;
 ...
 */
XedTab *
xed_window_get_tab_from_uri (XedWindow *window, const gchar *uri)
{
    GFile *f;
    XedTab *tab;
