			     ...
			     GParamSpec   *pspec)
{
	...
	{
		case PROP_ENABLE_SEARCH_HIGHLIGHTING:
			xed_document_set_enable_search_highlighting (doc,
								       g_value_get_boolean (value));
			break;
		case PROP_NEWLINE_TYPE:
			xed_document_set_newline_type (doc,
							 g_value_get_enum (value));
			break;
		case PROP_SHORTNAME:
			xed_document_set_short_name_for_display (doc,
			                                           g_value_get_string (value));
			break;
		case PROP_CONTENT_TYPE:
			xed_document_set_content_type (doc,
			                                 g_value_get_string (value));
			break;
		default:
			G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
                         ...
                         GtkTextMark       *mark)
{
	...
	if (GTK_TEXT_BUFFER_CLASS (xed_document_parent_class)->mark_set)
		GTK_TEXT_BUFFER_CLASS (xed_document_parent_class)->mark_set (buffer,
									       iter,
									       mark);
	      ...
	      gboolean             set_by_user)
{
	...
	if (set_by_user)
	{
		...
		xed_document_set_metadata (doc, XED_METADATA_ATTRIBUTE_ENCODING,
					     charset, NULL);
	}
			...
			XedDocument       *doc)
{
	     ...
	     error->code == XED_DOCUMENT_ERROR_CONVERSION_FALLBACK))
	{
		...
		set_encoding (doc, 
			      xed_document_loader_get_encoding (loader),
			      (doc->priv->requested_encoding != NULL));

		set_content_type (doc, content_type);

		xed_document_set_newline_type (doc,
		                                 xed_document_loader_get_newline_type (loader));
			  ...
			  XedDocumentSaveFlags  flags)
{
	...
	doc->priv->saver = xed_document_saver_new (doc, uri, encoding,
	...
	xed_document_saver_save (doc->priv->saver,
				   &doc->priv->mtime);
}
			...
			XedDocumentSaveFlags  flags)
{
	...
	g_signal_emit (doc,
		       ...
		       uri,
		       encoding,
		       flags | XED_DOCUMENT_SAVE_IGNORE_MTIME);
}
			       ...
			       const GtkTextIter *end)
{
	...
	*/
	...
	region = xed_text_region_intersect (doc->priv->to_search_region, 
					      start,
					      end);
	if (region) 
	{
		...
		xed_text_region_nth_subregion (region, 
						 0,
						 &start_search,
						 NULL);
		...
		xed_text_region_subtract (doc->priv->to_search_region,
					    start, 
					    end);
	}
}
					       ...
					       gboolean       enable)
{
	...
	if (doc->priv->to_search_region != NULL)
	{
		...
		xed_text_region_destroy (doc->priv->to_search_region,
					   TRUE);
			     ...
			     ...)
{
	...
	for (key = first_key; key; key = va_arg (var_args, const gchar *))
	{
		...
		xed_metadata_manager_set (doc->priv->uri,
					    key,
					    value);
	}
