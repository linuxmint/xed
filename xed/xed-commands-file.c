		...
		gboolean             create)
{
	...
	while (l != NULL)
	{
		...
		// FIXME: pass the GFile to tab when api is there
		uri = g_file_get_uri (l->data);
		tab = xed_window_create_tab_from_uri (window,
							uri,
							encoding,
							line_pos,
							create,
                         ...
                         XedWindow            *window)
{
	...
	xed_commands_load_files (window,
				   files,
				   encoding,
				   0);
                         ...
                         XedWindow            *window)
{
	...
	tab = XED_TAB (g_object_get_data (G_OBJECT (dialog),
					    XED_TAB_TO_SAVE_AS));
	      ...
	      XedWindow *window)
{
	...
	xed_file_chooser_dialog_set_encoding (XED_FILE_CHOOSER_DIALOG (save_dialog),
						encoding);

	xed_file_chooser_dialog_set_newline_type (XED_FILE_CHOOSER_DIALOG (save_dialog),
	                                            newline_type);
			      ...
			      XedWindow  *window)
{
	...
	while (sl != NULL)
	{
		save_and_close (XED_TAB (sl->data),
				window);
		sl = g_slist_next (sl);
	}
