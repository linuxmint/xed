 ...
 * xed-rsort-plugin.c
 *
 * Author: Martin Bertrand <xed@mbertrand.ca>
 *
 * Based on xed-sort-plugin.c by:
...
#include "xed-rsort-plugin.h"

#define MENU_PATH "/MenuBar/EditMenu/EditOps_7"

static void xed_window_activatable_iface_init (XedWindowActivatableInterface *iface);

struct _XedRSortPluginPrivate
{
    XedWindow *window;
...
G_DEFINE_DYNAMIC_TYPE_EXTENDED (XedRSortPlugin,
                                xed_rsort_plugin,
                                PEAS_TYPE_EXTENSION_BASE,
                                0,
                                G_IMPLEMENT_INTERFACE_DYNAMIC (XED_TYPE_WINDOW_ACTIVATABLE,
                                                               xed_window_activatable_iface_init)
                                G_ADD_PRIVATE_DYNAMIC (XedRSortPlugin))

static void rsort_cb (GtkAction     *action,
                     XedRSortPlugin *plugin);

static void buffer_rsort_lines (GtkSourceBuffer *buffer,
                               GtkTextIter     *start,
                               GtkTextIter     *end);
...
static const GtkActionEntry action_entries[] =
{
    { "RSort",
      ...
      N_("_RSort lines"),
      "<ctrl>F10",
      N_("Reverse Sort the current document or selection"),
      G_CALLBACK (rsort_cb)
    }
};
...
static void
get_current_selection (XedRSortPlugin *plugin)
{
          ...
          XedRSortPlugin *plugin)
{
    ...
    buffer_rsort_lines (GTK_SOURCE_BUFFER (doc),
                        &priv->start,
                        &priv->end);
}
...
compare_line (gconstpointer aptr,
              gconstpointer bptr)
{
    const RSortLine *a = aptr;
    const RSortLine *b = bptr;

    return g_strcmp0 (b->key, a->key);
}

static void
buffer_rsort_lines (GtkSourceBuffer    *buffer,
                    GtkTextIter        *start,
                    GtkTextIter        *end)
{
    ...
    lines = g_new0 (RSortLine, num_lines);
    ...
    qsort (lines, num_lines, sizeof (RSortLine), compare_line);
...
static void
update_ui (XedRSortPlugin *plugin)
{
    XedView *view;
...
static void
xed_rsort_plugin_activate (XedWindowActivatable *activatable)
{
    XedRSortPluginPrivate *priv;
    ...
    priv = XED_RSORT_PLUGIN (activatable)->priv;
    manager = xed_window_get_ui_manager (priv->window);

    priv->ui_action_group = gtk_action_group_new ("XedRSortPluginActions");
    ...
    update_ui (XED_RSORT_PLUGIN (activatable));
}

static void
xed_rsort_plugin_deactivate (XedWindowActivatable *activatable)
{
    XedRSortPluginPrivate *priv;
    GtkUIManager *manager;
...
static void
xed_rsort_plugin_update_state (XedWindowActivatable *activatable)
{
    xed_debug (DEBUG_PLUGINS);
...
static void
xed_rsort_plugin_init (XedRSortPlugin *plugin)
{
    xed_debug_message (DEBUG_PLUGINS, "XedRSortPlugin initializing");

    plugin->priv = xed_rsort_plugin_get_instance_private (plugin);
}

static void
xed_rsort_plugin_dispose (GObject *object)
{
    XedRSortPlugin *plugin = XED_RSORT_PLUGIN (object);

    xed_debug_message (DEBUG_PLUGINS, "XedRSortPlugin disposing");

    g_clear_object (&plugin->priv->ui_action_group);
    g_clear_object (&plugin->priv->window);

    G_OBJECT_CLASS (xed_rsort_plugin_parent_class)->dispose (object);
}

static void
xed_rsort_plugin_finalize (GObject *object)
{
    xed_debug_message (DEBUG_PLUGINS, "XedRSortPlugin finalizing");

    G_OBJECT_CLASS (xed_rsort_plugin_parent_class)->finalize (object);
}

static void
xed_rsort_plugin_set_property (GObject      *object,
                               guint         prop_id,
                               const GValue *value,
                               GParamSpec   *pspec)
{
...
static void
xed_rsort_plugin_get_property (GObject    *object,
                               guint       prop_id,
                               GValue     *value,
                               GParamSpec *pspec)
{
...
static void
xed_rsort_plugin_class_init (XedRSortPluginClass *klass)
{
    GObjectClass *object_class = G_OBJECT_CLASS (klass);

    object_class->dispose = xed_rsort_plugin_dispose;
    object_class->finalize = xed_rsort_plugin_finalize;
    object_class->set_property = xed_rsort_plugin_set_property;
    object_class->get_property = xed_rsort_plugin_get_property;

    g_object_class_override_property (object_class, PROP_WINDOW, "window");
}

static void
xed_rsort_plugin_class_finalize (XedRSortPluginClass *klass)
{
...
{
    iface->activate = xed_rsort_plugin_activate;
    iface->deactivate = xed_rsort_plugin_deactivate;
    iface->update_state = xed_rsort_plugin_update_state;
}

G_MODULE_EXPORT void
peas_register_types (PeasObjectModule *module)
{
    xed_rsort_plugin_register_type (G_TYPE_MODULE (module));

    peas_object_module_register_extension_type (module,
                                                XED_TYPE_WINDOW_ACTIVATABLE,
                                                XED_TYPE_RSORT_PLUGIN);
}
