/*
 * xedit-file-browser-store.c - Xedit plugin providing easy file access 
 * from the sidepanel
 *
...
#define XEDIT_FILE_BROWSER_STORE_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE((object), \
						     XEDIT_TYPE_FILE_BROWSER_STORE, \
						     XeditFileBrowserStorePrivate))
...
struct _XeditFileBrowserStorePrivate 
{
	FileBrowserNode *root;
	FileBrowserNode *virtual_root;
	GType column_types[XEDIT_FILE_BROWSER_STORE_COLUMN_NUM];

	XeditFileBrowserStoreFilterMode filter_mode;
	XeditFileBrowserStoreFilterFunc filter_func;
...
static FileBrowserNode *model_find_node 		    (XeditFileBrowserStore *model,
							     FileBrowserNode *node,
							     GFile *uri);
static void model_remove_node                               (XeditFileBrowserStore * model,
							     FileBrowserNode * node, 
							     GtkTreePath * path,
							     gboolean free_nodes);

static void set_virtual_root_from_node                      (XeditFileBrowserStore * model,
				                             FileBrowserNode * node);

static void xedit_file_browser_store_iface_init             (GtkTreeModelIface * iface);
static GtkTreeModelFlags xedit_file_browser_store_get_flags (GtkTreeModel * tree_model);
static gint xedit_file_browser_store_get_n_columns          (GtkTreeModel * tree_model);
static GType xedit_file_browser_store_get_column_type       (GtkTreeModel * tree_model,
							     gint index);
static gboolean xedit_file_browser_store_get_iter           (GtkTreeModel * tree_model,
							     GtkTreeIter * iter,
							     GtkTreePath * path);
static GtkTreePath *xedit_file_browser_store_get_path       (GtkTreeModel * tree_model,
							     GtkTreeIter * iter);
static void xedit_file_browser_store_get_value              (GtkTreeModel * tree_model,
							     GtkTreeIter * iter,
							     gint column,
							     GValue * value);
static gboolean xedit_file_browser_store_iter_next          (GtkTreeModel * tree_model,
							     GtkTreeIter * iter);
static gboolean xedit_file_browser_store_iter_children      (GtkTreeModel * tree_model,
							     GtkTreeIter * iter,
							     GtkTreeIter * parent);
static gboolean xedit_file_browser_store_iter_has_child     (GtkTreeModel * tree_model,
							     GtkTreeIter * iter);
static gint xedit_file_browser_store_iter_n_children        (GtkTreeModel * tree_model,
							     GtkTreeIter * iter);
static gboolean xedit_file_browser_store_iter_nth_child     (GtkTreeModel * tree_model,
							     GtkTreeIter * iter,
							     GtkTreeIter * parent, 
							     gint n);
static gboolean xedit_file_browser_store_iter_parent        (GtkTreeModel * tree_model,
							     GtkTreeIter * iter,
							     GtkTreeIter * child);
static void xedit_file_browser_store_row_inserted	    (GtkTreeModel * tree_model,
							     GtkTreePath * path,
							     GtkTreeIter * iter);

static void xedit_file_browser_store_drag_source_init       (GtkTreeDragSourceIface * iface);
static gboolean xedit_file_browser_store_row_draggable      (GtkTreeDragSource * drag_source,
							     GtkTreePath       * path);
static gboolean xedit_file_browser_store_drag_data_delete   (GtkTreeDragSource * drag_source,
							     GtkTreePath       * path);
static gboolean xedit_file_browser_store_drag_data_get      (GtkTreeDragSource * drag_source,
							     GtkTreePath       * path,
							     GtkSelectionData  * selection_data);

static void file_browser_node_free                          (XeditFileBrowserStore * model,
							     FileBrowserNode * node);
static void model_add_node                                  (XeditFileBrowserStore * model,
							     FileBrowserNode * child,
							     FileBrowserNode * parent);
static void model_clear                                     (XeditFileBrowserStore * model,
							     gboolean free_nodes);
static gint model_sort_default                              (FileBrowserNode * node1,
							     FileBrowserNode * node2);
static void model_check_dummy                               (XeditFileBrowserStore * model,
							     FileBrowserNode * node);
static void next_files_async 				    (GFileEnumerator * enumerator,
							     AsyncNode * async);

XEDIT_PLUGIN_DEFINE_TYPE_WITH_CODE (XeditFileBrowserStore, xedit_file_browser_store,
			G_TYPE_OBJECT,
			XEDIT_PLUGIN_IMPLEMENT_INTERFACE (xedit_file_browser_store_tree_model,
							  GTK_TYPE_TREE_MODEL,
							  xedit_file_browser_store_iface_init)
			XEDIT_PLUGIN_IMPLEMENT_INTERFACE (xedit_file_browser_store_drag_source,
							  GTK_TYPE_TREE_DRAG_SOURCE,
							  xedit_file_browser_store_drag_source_init))
...
static void
cancel_mount_operation (XeditFileBrowserStore *obj)
{
...
static void
xedit_file_browser_store_finalize (GObject * object)
{
	XeditFileBrowserStore *obj = XEDIT_FILE_BROWSER_STORE (object);
	GSList *item;
	...
	G_OBJECT_CLASS (xedit_file_browser_store_parent_class)->finalize (object);
}
...
static void
xedit_file_browser_store_get_property (GObject    *object,
			               guint       prop_id,
			               GValue     *value,
			               GParamSpec *pspec)
{
...
static void
xedit_file_browser_store_set_property (GObject      *object,
			               guint         prop_id,
			               const GValue *value,
			               GParamSpec   *pspec)
{
	...
	{
		case PROP_FILTER_MODE:
			xedit_file_browser_store_set_filter_mode (obj,
...
static void
xedit_file_browser_store_class_init (XeditFileBrowserStoreClass * klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	object_class->finalize = xedit_file_browser_store_finalize;

	object_class->get_property = xedit_file_browser_store_get_property;
	object_class->set_property = xedit_file_browser_store_set_property;
	...
	g_object_class_install_property (object_class, PROP_FILTER_MODE,
					 g_param_spec_flags ("filter-mode",
					 		      "Filter Mode",
					 		      "The filter mode",
					 		      XEDIT_TYPE_FILE_BROWSER_STORE_FILTER_MODE,
	...
	model_signals[BEGIN_LOADING] =
	    g_signal_new ("begin-loading",
			  ...
			  G_SIGNAL_RUN_LAST,
			  G_STRUCT_OFFSET (XeditFileBrowserStoreClass,
	...
	model_signals[NO_TRASH] =
	    g_signal_new ("no-trash", G_OBJECT_CLASS_TYPE (object_class),
			  ...
			  xedit_file_browser_marshal_BOOL__POINTER,
	...
	model_signals[RENAME] =
	    g_signal_new ("rename",
			  ...
			  G_STRUCT_OFFSET (XeditFileBrowserStoreClass,
					   rename), NULL, NULL,
			  xedit_file_browser_marshal_VOID__STRING_STRING, 
			  G_TYPE_NONE, 2,
			  G_TYPE_STRING,
			  G_TYPE_STRING);
	...
	g_type_class_add_private (object_class,
				  sizeof (XeditFileBrowserStorePrivate));
}

static void
xedit_file_browser_store_iface_init (GtkTreeModelIface * iface)
{
	iface->get_flags = xedit_file_browser_store_get_flags;
	iface->get_n_columns = xedit_file_browser_store_get_n_columns;
	iface->get_column_type = xedit_file_browser_store_get_column_type;
	iface->get_iter = xedit_file_browser_store_get_iter;
	iface->get_path = xedit_file_browser_store_get_path;
	iface->get_value = xedit_file_browser_store_get_value;
	iface->iter_next = xedit_file_browser_store_iter_next;
	iface->iter_children = xedit_file_browser_store_iter_children;
	iface->iter_has_child = xedit_file_browser_store_iter_has_child;
	iface->iter_n_children = xedit_file_browser_store_iter_n_children;
	iface->iter_nth_child = xedit_file_browser_store_iter_nth_child;
	iface->iter_parent = xedit_file_browser_store_iter_parent;
	iface->row_inserted = xedit_file_browser_store_row_inserted;
}

static void
xedit_file_browser_store_drag_source_init (GtkTreeDragSourceIface * iface)
{
	iface->row_draggable = xedit_file_browser_store_row_draggable;
	iface->drag_data_delete = xedit_file_browser_store_drag_data_delete;
	iface->drag_data_get = xedit_file_browser_store_drag_data_get;
}

static void
xedit_file_browser_store_init (XeditFileBrowserStore * obj)
{
	obj->priv = XEDIT_FILE_BROWSER_STORE_GET_PRIVATE (obj);

	obj->priv->column_types[XEDIT_FILE_BROWSER_STORE_COLUMN_URI] =
	    G_TYPE_STRING;
	obj->priv->column_types[XEDIT_FILE_BROWSER_STORE_COLUMN_NAME] =
	    G_TYPE_STRING;
	obj->priv->column_types[XEDIT_FILE_BROWSER_STORE_COLUMN_FLAGS] =
	    G_TYPE_UINT;
	obj->priv->column_types[XEDIT_FILE_BROWSER_STORE_COLUMN_ICON] =
	    GDK_TYPE_PIXBUF;
	obj->priv->column_types[XEDIT_FILE_BROWSER_STORE_COLUMN_EMBLEM] =
	    GDK_TYPE_PIXBUF;

	// Default filter mode is hiding the hidden files
	obj->priv->filter_mode = xedit_file_browser_store_filter_mode_get_default ();
...
static gboolean
node_in_tree (XeditFileBrowserStore * model, FileBrowserNode * node)
{
	return node_has_parent (node, model->priv->virtual_root);
}

static gboolean
model_node_visibility (XeditFileBrowserStore * model,
		       FileBrowserNode * node)
{
	if (node == NULL)
		return FALSE;
...
static gboolean
model_node_inserted (XeditFileBrowserStore * model,
		     FileBrowserNode * node)
{
...
xedit_file_browser_store_get_flags (GtkTreeModel * tree_model)
{
	g_return_val_if_fail (XEDIT_IS_FILE_BROWSER_STORE (tree_model),
			      (GtkTreeModelFlags) 0);
...
static gint
xedit_file_browser_store_get_n_columns (GtkTreeModel * tree_model)
{
	g_return_val_if_fail (XEDIT_IS_FILE_BROWSER_STORE (tree_model), 0);

	return XEDIT_FILE_BROWSER_STORE_COLUMN_NUM;
}

static GType
xedit_file_browser_store_get_column_type (GtkTreeModel * tree_model, gint idx)
{
	g_return_val_if_fail (XEDIT_IS_FILE_BROWSER_STORE (tree_model),
			      G_TYPE_INVALID);
	g_return_val_if_fail (idx < XEDIT_FILE_BROWSER_STORE_COLUMN_NUM &&
			      idx >= 0, G_TYPE_INVALID);

	return XEDIT_FILE_BROWSER_STORE (tree_model)->priv->column_types[idx];
}

static gboolean
xedit_file_browser_store_get_iter (GtkTreeModel * tree_model,
				   GtkTreeIter * iter, GtkTreePath * path)
{
	...
	g_assert (XEDIT_IS_FILE_BROWSER_STORE (tree_model));
	g_assert (path != NULL);

	model = XEDIT_FILE_BROWSER_STORE (tree_model);
...
static GtkTreePath *
xedit_file_browser_store_get_path_real (XeditFileBrowserStore * model,
					FileBrowserNode * node)
{
	GtkTreePath *path;
	gint num = 0;
...
static GtkTreePath *
xedit_file_browser_store_get_path (GtkTreeModel * tree_model,
				   GtkTreeIter * iter)
{
	g_return_val_if_fail (XEDIT_IS_FILE_BROWSER_STORE (tree_model), NULL);
	g_return_val_if_fail (iter != NULL, NULL);
	g_return_val_if_fail (iter->user_data != NULL, NULL);

	return xedit_file_browser_store_get_path_real (XEDIT_FILE_BROWSER_STORE (tree_model), 
						       (FileBrowserNode *) (iter->user_data));
}

static void
xedit_file_browser_store_get_value (GtkTreeModel * tree_model,
				    GtkTreeIter * iter, 
				    gint column,
				    GValue * value)
{
	...
	g_return_if_fail (XEDIT_IS_FILE_BROWSER_STORE (tree_model));
	g_return_if_fail (iter != NULL);
	g_return_if_fail (iter->user_data != NULL);

	node = (FileBrowserNode *) (iter->user_data);

	g_value_init (value, XEDIT_FILE_BROWSER_STORE (tree_model)->priv->column_types[column]);

	switch (column) {
	case XEDIT_FILE_BROWSER_STORE_COLUMN_URI:
		set_gvalue_from_node (value, node);
		break;
	case XEDIT_FILE_BROWSER_STORE_COLUMN_NAME:
		g_value_set_string (value, node->name);
		break;
	case XEDIT_FILE_BROWSER_STORE_COLUMN_FLAGS:
		g_value_set_uint (value, node->flags);
		break;
	case XEDIT_FILE_BROWSER_STORE_COLUMN_ICON:
		g_value_set_object (value, node->icon);
		break;
	case XEDIT_FILE_BROWSER_STORE_COLUMN_EMBLEM:
...
static gboolean
xedit_file_browser_store_iter_next (GtkTreeModel * tree_model,
				    GtkTreeIter * iter)
{
	XeditFileBrowserStore * model;
	FileBrowserNode * node;
...
static gboolean
xedit_file_browser_store_iter_children (GtkTreeModel * tree_model,
					GtkTreeIter * iter,
					GtkTreeIter * parent)
{
	FileBrowserNode * node;
...
static gboolean
filter_tree_model_iter_has_child_real (XeditFileBrowserStore * model,
				       FileBrowserNode * node)
{
	GSList *item;
...
static gboolean
xedit_file_browser_store_iter_has_child (GtkTreeModel * tree_model,
					 GtkTreeIter * iter)
{
	FileBrowserNode *node;
	XeditFileBrowserStore *model;
...
static gint
xedit_file_browser_store_iter_n_children (GtkTreeModel * tree_model,
					  GtkTreeIter * iter)
{
	FileBrowserNode *node;
	XeditFileBrowserStore *model;
...
static gboolean
xedit_file_browser_store_iter_nth_child (GtkTreeModel * tree_model,
					 GtkTreeIter * iter,
...
static gboolean
xedit_file_browser_store_iter_parent (GtkTreeModel * tree_model,
				      GtkTreeIter * iter,
				      GtkTreeIter * child)
{
	FileBrowserNode *node;
	...
	g_return_val_if_fail (XEDIT_IS_FILE_BROWSER_STORE (tree_model), FALSE);
...
static void
xedit_file_browser_store_row_inserted (GtkTreeModel * tree_model,
				       GtkTreePath * path,
				       GtkTreeIter * iter)
{
...
static gboolean
xedit_file_browser_store_drag_data_delete (GtkTreeDragSource * drag_source,
					   GtkTreePath       * path)
{
	return FALSE;
}
...
static gboolean
xedit_file_browser_store_drag_data_get (GtkTreeDragSource * drag_source,
					GtkTreePath       * path,
					GtkSelectionData  * selection_data)
{
	GtkTreeIter iter;
...
#define FILTER_HIDDEN(mode) (mode & XEDIT_FILE_BROWSER_STORE_FILTER_MODE_HIDE_HIDDEN)
#define FILTER_BINARY(mode) (mode & XEDIT_FILE_BROWSER_STORE_FILTER_MODE_HIDE_BINARY)

/* Private */
static void
model_begin_loading (XeditFileBrowserStore * model, FileBrowserNode * node)
{
	GtkTreeIter iter;
...
static void
model_end_loading (XeditFileBrowserStore * model, FileBrowserNode * node)
{
	GtkTreeIter iter;
...
static void
model_node_update_visibility (XeditFileBrowserStore * model,
			      FileBrowserNode * node)
{
	GtkTreeIter iter;

	node->flags &= ~XEDIT_FILE_BROWSER_STORE_FLAG_IS_FILTERED;
	...
	if (FILTER_HIDDEN (model->priv->filter_mode) &&
		...
		if (!model->priv->
			...
			node->flags |=
			    XEDIT_FILE_BROWSER_STORE_FLAG_IS_FILTERED;
	}
}
...
static void
model_resort_node (XeditFileBrowserStore * model, FileBrowserNode * node)
{
	FileBrowserNodeDir *dir;
	GSList *item;
	FileBrowserNode *child;
	...
	if (!model_node_visibility (model, node->parent)) {
		...
		iter.user_data = node->parent;
		path =
		    xedit_file_browser_store_get_path_real (model,
							    node->parent);
...
static void
row_changed (XeditFileBrowserStore * model,
	     GtkTreePath ** path,
	     GtkTreeIter * iter)
{
...
static void
row_inserted (XeditFileBrowserStore * model,
	      GtkTreePath ** path,
	      GtkTreeIter * iter)
{
...
static void
row_deleted (XeditFileBrowserStore * model,
...
static void
model_refilter_node (XeditFileBrowserStore * model,
		     FileBrowserNode * node,
		     GtkTreePath ** path)
{
	...
	{
		if (in_tree)
			tmppath = xedit_file_browser_store_get_path_real (model,
								       node);
		else
...
static void
model_refilter (XeditFileBrowserStore * model)
{
...
file_browser_node_set_name (FileBrowserNode * node)
{
	...
	if (node->file) {
		node->name = xedit_file_browser_utils_file_basename (node->file);
	} else {
		node->name = NULL;
	}
}
...
static FileBrowserNode *
file_browser_node_dir_new (XeditFileBrowserStore * model,
			   GFile * file, FileBrowserNode * parent)
{
	...
	node->flags |= XEDIT_FILE_BROWSER_STORE_FLAG_IS_DIRECTORY;
...
static void
file_browser_node_free_children (XeditFileBrowserStore * model,
				 FileBrowserNode * node)
{
	GSList *item;
	...
	if (NODE_IS_DIR (node)) {
		...
		node->flags &= ~XEDIT_FILE_BROWSER_STORE_FLAG_LOADED;
	}
}

static void
file_browser_node_free (XeditFileBrowserStore * model,
			FileBrowserNode * node)
{
	gchar *uri;
 ...
 * @model: the #XeditFileBrowserStore
 ...
 **/
static void
model_remove_node_children (XeditFileBrowserStore * model,
			    FileBrowserNode * node,
			    GtkTreePath * path,
			    gboolean free_nodes)
{
	...
	if (path == NULL)
		path_child =
		    xedit_file_browser_store_get_path_real (model, node);
	else
 ...
 **/
static void
model_remove_node (XeditFileBrowserStore * model,
		   FileBrowserNode * node,
		   GtkTreePath * path,
		   gboolean free_nodes)
{
 ...
 **/
static void
model_clear (XeditFileBrowserStore * model, gboolean free_nodes)
{
	GtkTreePath *path;
	FileBrowserNodeDir *dir;
...
static void
file_browser_node_unload (XeditFileBrowserStore * model,
...
static void
model_recomposite_icon_real (XeditFileBrowserStore * tree_model,
			     FileBrowserNode * node,
			     GFileInfo * info)
{
	GdkPixbuf *icon;
	...
	if (info) {
		...
		if (gicon != NULL)
			icon = xedit_file_browser_utils_pixbuf_from_icon (gicon, GTK_ICON_SIZE_MENU);
		else
			icon = NULL;
	} else {
		icon = xedit_file_browser_utils_pixbuf_from_file (node->file, GTK_ICON_SIZE_MENU);
	}
...
static void
model_recomposite_icon (XeditFileBrowserStore * tree_model,
			GtkTreeIter * iter)
{
	g_return_if_fail (XEDIT_IS_FILE_BROWSER_STORE (tree_model));
	g_return_if_fail (iter != NULL);
...
static FileBrowserNode *
model_create_dummy_node (XeditFileBrowserStore * model,
			 FileBrowserNode * parent)
{
	FileBrowserNode *dummy;
	...
	dummy->flags |= XEDIT_FILE_BROWSER_STORE_FLAG_IS_DUMMY;
	dummy->flags |= XEDIT_FILE_BROWSER_STORE_FLAG_IS_HIDDEN;

	return dummy;
}

static FileBrowserNode *
model_add_dummy_node (XeditFileBrowserStore * model,
		      FileBrowserNode * parent)
{
	FileBrowserNode *dummy;
...
static void
model_check_dummy (XeditFileBrowserStore * model, FileBrowserNode * node)
{
	...
	if (NODE_IS_DIR (node)) {
		...
		if (!model_node_visibility (model, node)) {
			dummy->flags |=
			    XEDIT_FILE_BROWSER_STORE_FLAG_IS_HIDDEN;
			return;
		}
		...
		if (!filter_tree_model_iter_has_child_real (model, node)) {
			...
			if (FILE_IS_HIDDEN (flags)) {
				...
				path =
				    xedit_file_browser_store_get_path_real
				    (model, dummy);
...
static void
insert_node_sorted (XeditFileBrowserStore * model,
		    FileBrowserNode * child,
		    FileBrowserNode * parent)
{
	FileBrowserNodeDir *dir;
...
static void
model_add_node (XeditFileBrowserStore * model, FileBrowserNode * child,
		FileBrowserNode * parent)
{
	...
	if (model_node_visibility (model, parent) &&
		...
		path = xedit_file_browser_store_get_path_real (model, child);
...
static void
model_add_nodes_batch (XeditFileBrowserStore * model,
		       GSList * children,
		       FileBrowserNode * parent)
{
	...
	while (child) {
		...
		if (l == NULL) {
			...
			for (l = child; l; l = l->next) {
				if (model_node_visibility (model, parent) &&
					...
					path = xedit_file_browser_store_get_path_real (model, l->data);
		...
		if (model->priv->sort_func (l->data, node) > 0) {
			...
			if (model_node_visibility (model, parent) &&
				...
				iter.user_data = node;
				path = xedit_file_browser_store_get_path_real (model, node);
...
static void
file_browser_node_set_from_info (XeditFileBrowserStore * model,
				 FileBrowserNode * node,
				 GFileInfo * info,
				 gboolean isadded)
{
	...
	if (g_file_info_get_is_hidden (info) || g_file_info_get_is_backup (info))
		node->flags |= XEDIT_FILE_BROWSER_STORE_FLAG_IS_HIDDEN;
	...
	if (g_file_info_get_file_type (info) == G_FILE_TYPE_DIRECTORY)
		...
		if (!content || 
			...
			node->flags |= XEDIT_FILE_BROWSER_STORE_FLAG_IS_TEXT;		
	}
...
static FileBrowserNode *
model_add_node_from_file (XeditFileBrowserStore * model,
			  FileBrowserNode * parent,
			  GFile * file,
			  GFileInfo * info)
{
 ...
 * added */
static void
model_add_nodes_from_files (XeditFileBrowserStore * model,
			    FileBrowserNode * parent,
			    GSList * original_children,
			    GList * files)
{
...
static FileBrowserNode *
model_add_node_from_dir (XeditFileBrowserStore * model,
			 FileBrowserNode * parent,
			 GFile * file)
{
	FileBrowserNode *node;
	...
	if ((node = node_list_contains_file (FILE_BROWSER_NODE_DIR (parent)->children, file)) == NULL) {	
		...
		if (node->icon == NULL) {
			node->icon = xedit_file_browser_utils_pixbuf_from_theme ("folder", GTK_ICON_SIZE_MENU);
		}
...
static void
model_load_directory (XeditFileBrowserStore * model,
		      FileBrowserNode * node)
{
	FileBrowserNodeDir *dir;
	AsyncNode *async;
...
static GList *
get_parent_files (XeditFileBrowserStore * model, GFile * file)
{
	GList * result = NULL;
...
static void
model_fill (XeditFileBrowserStore * model, FileBrowserNode * node,
	    GtkTreePath ** path)
{
	gboolean free_path = FALSE;
	GtkTreeIter iter = {0,};
...
static void
set_virtual_root_from_node (XeditFileBrowserStore * model,
			    FileBrowserNode * node)
{
...
static void
set_virtual_root_from_file (XeditFileBrowserStore * model,
			    GFile * file)
{
	GList * files;
	GList * item;
...
static FileBrowserNode *
model_find_node_children (XeditFileBrowserStore * model,
			  FileBrowserNode * parent,
			  GFile * file)
{
	FileBrowserNodeDir *dir;
...
static FileBrowserNode *
model_find_node (XeditFileBrowserStore * model,
		 FileBrowserNode * node,
		 GFile * file)
{
	if (node == NULL)
...
static GQuark
xedit_file_browser_store_error_quark (void)
{
	static GQuark quark = 0;

	if (G_UNLIKELY (quark == 0)) {
		quark = g_quark_from_string ("xedit_file_browser_store_error");
	}
...
static XeditFileBrowserStoreResult
model_root_mounted (XeditFileBrowserStore * model, gchar const * virtual_root)
{
	...
	if (virtual_root != NULL)
		return
		    xedit_file_browser_store_set_virtual_root_from_string
		    (model, virtual_root);
	else
		set_virtual_root_from_node (model,
					    model->priv->root);

	return XEDIT_FILE_BROWSER_STORE_RESULT_OK;
}

static void
handle_root_error (XeditFileBrowserStore * model, GError *error)
{
	FileBrowserNode * root;
	...
	root->flags |= XEDIT_FILE_BROWSER_STORE_FLAG_LOADED;
	  ...
	  MountInfo * mount_info)
{
	gboolean mounted;
	GError * error = NULL;
	XeditFileBrowserStore * model = mount_info->model;
...
static XeditFileBrowserStoreResult
model_mount_root (XeditFileBrowserStore * model, gchar const * virtual_root)
{
	GFileInfo * info;
	GError * error = NULL;
	MountInfo * mount_info;
	...
	if (!info) {
		if (error->code == G_IO_ERROR_NOT_MOUNTED) {
			...
			return XEDIT_FILE_BROWSER_STORE_RESULT_MOUNTING;
		}
		else
		{
			handle_root_error (model, error);
...
xedit_file_browser_store_new (gchar const *root)
{
	XeditFileBrowserStore *obj =
	    XEDIT_FILE_BROWSER_STORE (g_object_new
				      (XEDIT_TYPE_FILE_BROWSER_STORE,
				       NULL));

	xedit_file_browser_store_set_root (obj, root);
	return obj;
}

void
xedit_file_browser_store_set_value (XeditFileBrowserStore * tree_model,
				    ...
				    GValue * value)
{
	...
	if (model_node_visibility (tree_model, node)) {
		path = xedit_file_browser_store_get_path (GTK_TREE_MODEL (tree_model), 
							  iter);
...
xedit_file_browser_store_set_virtual_root (XeditFileBrowserStore * model,
					   GtkTreeIter * iter)
{
	g_return_val_if_fail (XEDIT_IS_FILE_BROWSER_STORE (model),
...
XeditFileBrowserStoreResult
xedit_file_browser_store_set_virtual_root_from_string
    (XeditFileBrowserStore * model, gchar const *root) {
	GFile *file;
	...
	if (file == NULL) {
		...
		return XEDIT_FILE_BROWSER_STORE_RESULT_NO_CHANGE;
	}
	...
	if (!g_file_has_prefix (file, model->priv->root->file)) {
		...
		g_object_unref (file);
		return XEDIT_FILE_BROWSER_STORE_RESULT_ERROR;
	}
...
XeditFileBrowserStoreResult
xedit_file_browser_store_set_virtual_root_top (XeditFileBrowserStore *
					       model)
{
	g_return_val_if_fail (XEDIT_IS_FILE_BROWSER_STORE (model),
			      XEDIT_FILE_BROWSER_STORE_RESULT_NO_CHANGE);
...
XeditFileBrowserStoreResult
xedit_file_browser_store_set_virtual_root_up (XeditFileBrowserStore *
					      model)
{
	g_return_val_if_fail (XEDIT_IS_FILE_BROWSER_STORE (model),
			      XEDIT_FILE_BROWSER_STORE_RESULT_NO_CHANGE);
...
gboolean
xedit_file_browser_store_get_iter_virtual_root (XeditFileBrowserStore *
						model, GtkTreeIter * iter)
{
	g_return_val_if_fail (XEDIT_IS_FILE_BROWSER_STORE (model), FALSE);
	g_return_val_if_fail (iter != NULL, FALSE);
...
gboolean
xedit_file_browser_store_get_iter_root (XeditFileBrowserStore * model,
					GtkTreeIter * iter)
{
	g_return_val_if_fail (XEDIT_IS_FILE_BROWSER_STORE (model), FALSE);
	g_return_val_if_fail (iter != NULL, FALSE);
...
gboolean
xedit_file_browser_store_iter_equal (XeditFileBrowserStore * model,
				     GtkTreeIter * iter1,
				     GtkTreeIter * iter2)
{
	g_return_val_if_fail (XEDIT_IS_FILE_BROWSER_STORE (model), FALSE);
...
void
xedit_file_browser_store_cancel_mount_operation (XeditFileBrowserStore *store)
{
	g_return_if_fail (XEDIT_IS_FILE_BROWSER_STORE (store));
	
	cancel_mount_operation (store);
}

XeditFileBrowserStoreResult
xedit_file_browser_store_set_root_and_virtual_root (XeditFileBrowserStore *
						    model,
						    gchar const *root,
				   ...
				   gchar const *root)
{
	g_return_val_if_fail (XEDIT_IS_FILE_BROWSER_STORE (model),
			      XEDIT_FILE_BROWSER_STORE_RESULT_NO_CHANGE);
	return xedit_file_browser_store_set_root_and_virtual_root (model,
								   root,
								   NULL);
}

gchar *
xedit_file_browser_store_get_root (XeditFileBrowserStore * model)
{
	g_return_val_if_fail (XEDIT_IS_FILE_BROWSER_STORE (model), NULL);
...
gchar * 
xedit_file_browser_store_get_virtual_root (XeditFileBrowserStore * model)
{
	g_return_val_if_fail (XEDIT_IS_FILE_BROWSER_STORE (model), NULL);
...
void
_xedit_file_browser_store_iter_expanded (XeditFileBrowserStore * model,
					 GtkTreeIter * iter)
{
	FileBrowserNode *node;

	g_return_if_fail (XEDIT_IS_FILE_BROWSER_STORE (model));
	g_return_if_fail (iter != NULL);
	g_return_if_fail (iter->user_data != NULL);
...
void
_xedit_file_browser_store_iter_collapsed (XeditFileBrowserStore * model,
					  GtkTreeIter * iter)
{
	FileBrowserNode *node;
	GSList *item;
...
void
xedit_file_browser_store_set_filter_mode (XeditFileBrowserStore * model,
					  XeditFileBrowserStoreFilterMode
					  mode)
{
	g_return_if_fail (XEDIT_IS_FILE_BROWSER_STORE (model));
...
void
xedit_file_browser_store_set_filter_func (XeditFileBrowserStore * model,
...
void
xedit_file_browser_store_refilter (XeditFileBrowserStore * model)
{
	model_refilter (model);
}
...
{
	return XEDIT_FILE_BROWSER_STORE_FILTER_MODE_HIDE_HIDDEN;
}

void
xedit_file_browser_store_refresh (XeditFileBrowserStore * model)
{
	g_return_if_fail (XEDIT_IS_FILE_BROWSER_STORE (model));
...
gboolean
xedit_file_browser_store_rename (XeditFileBrowserStore * model,
				 GtkTreeIter * iter,
				 ...
				 GError ** error)
{
	...
	if (g_file_move (node->file, file, G_FILE_COPY_NONE, NULL, NULL, NULL, &err)) {
		...
		if (model_node_visibility (model, node)) {
			...
			if (error != NULL)
				*error = g_error_new_literal (xedit_file_browser_store_error_quark (),
		...
		if (err) {
			if (error != NULL) {
				*error =
				    g_error_new_literal
				    (xedit_file_browser_store_error_quark (),
				     XEDIT_FILE_BROWSER_ERROR_RENAME,
				     err->message);
			}
				     ...
				     GList *rows, gboolean trash)
{
	...
	g_return_val_if_fail (XEDIT_IS_FILE_BROWSER_STORE (model), XEDIT_FILE_BROWSER_STORE_RESULT_NO_CHANGE);
				 ...
				 GtkTreeIter * iter, gboolean trash)
{
	FileBrowserNode *node;
	GList *rows = NULL;
	XeditFileBrowserStoreResult result;

	g_return_val_if_fail (XEDIT_IS_FILE_BROWSER_STORE (model), XEDIT_FILE_BROWSER_STORE_RESULT_NO_CHANGE);
	g_return_val_if_fail (iter != NULL, XEDIT_FILE_BROWSER_STORE_RESULT_NO_CHANGE);
	g_return_val_if_fail (iter->user_data != NULL, XEDIT_FILE_BROWSER_STORE_RESULT_NO_CHANGE);
	...
	rows = g_list_append(NULL, xedit_file_browser_store_get_path_real (model, node));
	result = xedit_file_browser_store_delete_all (model, rows, trash);
...
gboolean
xedit_file_browser_store_new_file (XeditFileBrowserStore * model,
				   GtkTreeIter * parent,
				   GtkTreeIter * iter)
{
	GFile * file;
...
gboolean
xedit_file_browser_store_new_directory (XeditFileBrowserStore * model,
					GtkTreeIter * parent,
					GtkTreeIter * iter)
{
	GFile * file;
