/*
 * xedit-file-browser-widget.c - Xedit plugin providing easy file access 
 * from the sidepanel
 *
...
#define XEDIT_FILE_BROWSER_WIDGET_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE((object), \
						      XEDIT_TYPE_FILE_BROWSER_WIDGET, \
						      XeditFileBrowserWidgetPrivate))

#define XML_UI_FILE "xedit-file-browser-widget-ui.xml"
#define LOCATION_DATA_KEY "xedit-file-browser-widget-location"
...
struct _XeditFileBrowserWidgetPrivate 
{
	XeditFileBrowserView *treeview;
	XeditFileBrowserStore *file_store;
	XeditFileBookmarksStore *bookmarks_store;
...
static void set_enable_delete		       (XeditFileBrowserWidget *obj,
						gboolean enable);
static void on_model_set                       (GObject * gobject, 
						GParamSpec * arg1,
						XeditFileBrowserWidget * obj);
static void on_treeview_error                  (XeditFileBrowserView * tree_view,
						guint code, 
						gchar * message,
						XeditFileBrowserWidget * obj);
static void on_file_store_error                (XeditFileBrowserStore * store, 
						guint code,
						gchar * message,
						XeditFileBrowserWidget * obj);
static gboolean on_file_store_no_trash 	       (XeditFileBrowserStore * store, 
						GList * files, 
						XeditFileBrowserWidget * obj);
static void on_combo_changed                   (GtkComboBox * combo,
						XeditFileBrowserWidget * obj);
static gboolean on_treeview_popup_menu         (XeditFileBrowserView * treeview,
						XeditFileBrowserWidget * obj);
static gboolean on_treeview_button_press_event (XeditFileBrowserView * treeview,
						GdkEventButton * event,
						XeditFileBrowserWidget * obj);
static gboolean on_treeview_key_press_event    (XeditFileBrowserView * treeview, 
						GdkEventKey * event,
						XeditFileBrowserWidget * obj);
static void on_selection_changed               (GtkTreeSelection * selection,
						XeditFileBrowserWidget * obj);

static void on_virtual_root_changed            (XeditFileBrowserStore * model,
						GParamSpec *param,
						XeditFileBrowserWidget * obj);

static gboolean on_entry_filter_activate       (XeditFileBrowserWidget * obj);
...
static void on_bookmarks_row_deleted           (GtkTreeModel * model, 
                                                GtkTreePath * path,
                                                XeditFileBrowserWidget * obj);
static void on_filter_mode_changed	       (XeditFileBrowserStore * model,
                                                GParamSpec * param,
                                                XeditFileBrowserWidget * obj);
...
#if GTK_CHECK_VERSION (3, 0, 0)
XEDIT_PLUGIN_DEFINE_TYPE (XeditFileBrowserWidget, xedit_file_browser_widget,
	                  GTK_TYPE_BOX)
#else
XEDIT_PLUGIN_DEFINE_TYPE (XeditFileBrowserWidget, xedit_file_browser_widget,
	                  GTK_TYPE_VBOX)
...
static FilterFunc *
filter_func_new (XeditFileBrowserWidget * obj,
		 XeditFileBrowserWidgetFilterFunc func,
		 gpointer user_data,
...
static gboolean
combo_find_by_id (XeditFileBrowserWidget * obj, guint id,
		  GtkTreeIter * iter)
{
	guint checkid;
...
static void
remove_path_items (XeditFileBrowserWidget * obj)
{
	GtkTreeIter iter;
...
static void
cancel_async_operation (XeditFileBrowserWidget *widget)
{
...
static void
xedit_file_browser_widget_finalize (GObject * object)
{
	XeditFileBrowserWidget *obj = XEDIT_FILE_BROWSER_WIDGET (object);
	GList *loc;

	remove_path_items (obj);
	xedit_file_browser_store_set_filter_func (obj->priv->file_store,
						  NULL, NULL);
	...
	G_OBJECT_CLASS (xedit_file_browser_widget_parent_class)->finalize (object);
}

static void
xedit_file_browser_widget_get_property (GObject    *object,
			               guint       prop_id,
			               GValue     *value,
			               GParamSpec *pspec)
{
...
static void
xedit_file_browser_widget_set_property (GObject      *object,
			               guint         prop_id,
			               const GValue *value,
			               GParamSpec   *pspec)
{
	...
	{
		case PROP_FILTER_PATTERN:
			xedit_file_browser_widget_set_filter_pattern (obj,
...
static void
xedit_file_browser_widget_class_init (XeditFileBrowserWidgetClass * klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	object_class->finalize = xedit_file_browser_widget_finalize;

	object_class->get_property = xedit_file_browser_widget_get_property;
	object_class->set_property = xedit_file_browser_widget_set_property;
	...
	signals[URI_ACTIVATED] =
	    g_signal_new ("uri-activated",
			  ...
			  G_SIGNAL_RUN_LAST,
			  G_STRUCT_OFFSET (XeditFileBrowserWidgetClass,
	...
	signals[CONFIRM_DELETE] =
	    g_signal_new ("confirm-delete", G_OBJECT_CLASS_TYPE (object_class),
	                  ...
	                  NULL,
	                  xedit_file_browser_marshal_BOOL__OBJECT_POINTER,
	                  G_TYPE_BOOLEAN,
	                  2,
	                  G_TYPE_OBJECT,
	                  G_TYPE_POINTER);
	...
	g_type_class_add_private (object_class,
				  sizeof (XeditFileBrowserWidgetPrivate));
}

static void
add_signal (XeditFileBrowserWidget * obj, gpointer object, gulong id)
{
...
static void
clear_signals (XeditFileBrowserWidget * obj)
{
	GSList *item;
	SignalNode *node;
...
static gboolean
get_from_bookmark_file (XeditFileBrowserWidget * obj, GFile * file,
...
static void
insert_path_item (XeditFileBrowserWidget * obj, 
                  GFile * file,
		  GtkTreeIter * after, 
		  GtkTreeIter * iter, 
		  guint indent)
{
	...
	if (!get_from_bookmark_file (obj, file, &unescape, &icon)) {
		...
		unescape = xedit_file_browser_utils_file_basename (file);
		
		/* Get the icon */
		icon = xedit_file_browser_utils_pixbuf_from_file (file, GTK_ICON_SIZE_MENU);
	}
...
static void
insert_separator_item (XeditFileBrowserWidget * obj)
{
	GtkTreeIter iter;
...
static void
combo_set_active_by_id (XeditFileBrowserWidget * obj, guint id)
{
	GtkTreeIter iter;
...
static void
insert_location_path (XeditFileBrowserWidget * obj)
{
	Location *loc;
	GFile *current = NULL;
	GFile * tmp;
	...
	while (current != NULL) {
		...
		if (g_file_equal (current, loc->root) || !xedit_utils_file_has_parent (current)) {
			if (current != loc->virtual_root)
...
static void
check_current_item (XeditFileBrowserWidget * obj, gboolean show_path)
{
	GtkTreeIter separator;
	gboolean has_sep;
...
static void
fill_combo_model (XeditFileBrowserWidget * obj)
{
	GtkTreeStore *store = obj->priv->combo_model;
	GtkTreeIter iter;
	GdkPixbuf *icon;

	icon = xedit_file_browser_utils_pixbuf_from_theme (GTK_STOCK_HOME, GTK_ICON_SIZE_MENU);
...
static void
create_combo (XeditFileBrowserWidget * obj)
{
...
static void
create_toolbar (XeditFileBrowserWidget * obj,
...
static void 
set_enable_delete (XeditFileBrowserWidget *obj,
		   gboolean enable)
{
	GtkAction *action;
...
static gboolean
filter_real (XeditFileBrowserStore * model, GtkTreeIter * iter,
	     XeditFileBrowserWidget * obj)
{
	GSList *item;
	FilterFunc *func;
...
static void
add_bookmark_hash (XeditFileBrowserWidget * obj,
                   GtkTreeIter * iter)
{
	GtkTreeModel *model;
	GdkPixbuf * pixbuf;
	...
	uri = xedit_file_bookmarks_store_get_uri (obj->priv->
						  bookmarks_store,
						  iter);
	...
	gtk_tree_model_get (model, iter,
			    XEDIT_FILE_BOOKMARKS_STORE_COLUMN_ICON,
			    &pixbuf,
...
static void
init_bookmarks_hash (XeditFileBrowserWidget * obj)
{
	GtkTreeIter iter;
	GtkTreeModel *model;
...
static void
on_begin_loading (XeditFileBrowserStore  *model, 
		  GtkTreeIter            *iter,
		  XeditFileBrowserWidget *obj)
{
...
static void
on_end_loading (XeditFileBrowserStore  *model, 
		GtkTreeIter            *iter,
		XeditFileBrowserWidget *obj)
{
	if (!GDK_IS_WINDOW (gtk_widget_get_window (GTK_WIDGET (obj->priv->treeview))))
...
static void
create_tree (XeditFileBrowserWidget * obj)
{
	GtkWidget *sw;

	obj->priv->file_store = xedit_file_browser_store_new (NULL);
	obj->priv->bookmarks_store = xedit_file_bookmarks_store_new ();
	obj->priv->treeview =
	    XEDIT_FILE_BROWSER_VIEW (xedit_file_browser_view_new ());

	xedit_file_browser_view_set_restore_expand_state (obj->priv->treeview, TRUE);

	xedit_file_browser_store_set_filter_mode (obj->priv->file_store,
...
static void
create_filter (XeditFileBrowserWidget * obj)
{
	GtkWidget *expander;
	GtkWidget *vbox;
	GtkWidget *entry;
...
static void
xedit_file_browser_widget_init (XeditFileBrowserWidget * obj)
{
#if GTK_CHECK_VERSION (3, 16, 0)
	GdkDisplay *display;
#endif
	obj->priv = XEDIT_FILE_BROWSER_WIDGET_GET_PRIVATE (obj);
...
static void
update_sensitivity (XeditFileBrowserWidget * obj)
{
	GtkTreeModel *model =
	...
	if (XEDIT_IS_FILE_BROWSER_STORE (model)) {
		...
		mode =
		    xedit_file_browser_store_get_filter_mode
		    (XEDIT_FILE_BROWSER_STORE (model));
		...
		gtk_toggle_action_set_active (GTK_TOGGLE_ACTION (action),
					      !(mode &
						XEDIT_FILE_BROWSER_STORE_FILTER_MODE_HIDE_HIDDEN));
	} else if (XEDIT_IS_FILE_BOOKMARKS_STORE (model)) {
		gtk_action_group_set_sensitive (obj->priv->action_group,
						FALSE);
		gtk_action_group_set_sensitive (obj->priv->bookmark_action_group,
						TRUE);
...
static gboolean
xedit_file_browser_widget_get_first_selected (XeditFileBrowserWidget *obj,
					      GtkTreeIter *iter)
{
...
static gboolean
popup_menu (XeditFileBrowserWidget * obj, GdkEventButton * event, GtkTreeModel * model)
{
	GtkWidget *menu;
...
static gboolean
filter_glob (XeditFileBrowserWidget * obj, XeditFileBrowserStore * store,
	     GtkTreeIter * iter, gpointer user_data)
{
	...
	gtk_tree_model_get (GTK_TREE_MODEL (store), iter,
			    XEDIT_FILE_BROWSER_STORE_COLUMN_NAME, &name,
...
static void
rename_selected_file (XeditFileBrowserWidget * obj)
{
	GtkTreeModel *model;
	GtkTreeIter iter;
	...
	if (xedit_file_browser_widget_get_first_selected (obj, &iter))
		xedit_file_browser_view_start_rename (obj->priv->treeview,
						      &iter);
}

static GList *
get_deletable_files (XeditFileBrowserWidget *obj) {
	GtkTreeSelection *selection;
	GtkTreeModel *model;
	GList *rows;
	GList *row;
...
static gboolean
delete_selected_files (XeditFileBrowserWidget * obj, gboolean trash)
{
	GtkTreeModel *model;
	gboolean confirm;
	...
	result = xedit_file_browser_store_delete_all (XEDIT_FILE_BROWSER_STORE (model),
						      rows, trash);
	
	g_list_foreach (rows, (GFunc)gtk_tree_path_free, NULL);
	g_list_free (rows);

	return result == XEDIT_FILE_BROWSER_STORE_RESULT_OK;
}

static gboolean
on_file_store_no_trash (XeditFileBrowserStore * store, 
			GList * files, 
			XeditFileBrowserWidget * obj)
{
...
static GtkWidget *
create_goto_menu_item (XeditFileBrowserWidget * obj, GList * item,
		       GdkPixbuf * icon)
{
	GtkWidget *result;
	GtkWidget *image;
	...
	if (!get_from_bookmark_file (obj, loc->virtual_root, &unescape, &pixbuf)) {
		unescape = xedit_file_browser_utils_file_basename (loc->virtual_root);
...
static void
jump_to_location (XeditFileBrowserWidget * obj, GList * item,
		  gboolean previous)
{
	Location *loc;
	GtkWidget *widget;
	...
	xedit_file_browser_widget_set_root_and_virtual_root (obj,
							     root,
							     virtual_root);
...
static void
clear_next_locations (XeditFileBrowserWidget * obj)
{
	GList *children;
	GList *item;
...
static void
update_filter_mode (XeditFileBrowserWidget * obj, 
                    GtkAction * action, 
                    XeditFileBrowserStoreFilterMode mode)
{
	...
	if (XEDIT_IS_FILE_BROWSER_STORE (model)) {
		...
		xedit_file_browser_store_set_filter_mode
		    (XEDIT_FILE_BROWSER_STORE (model), now);
	}
}

static void
set_filter_pattern_real (XeditFileBrowserWidget * obj,
                        gchar const * pattern,
                        gboolean update_entry)
{
	...
	if (pattern == NULL) {
		if (obj->priv->glob_filter_id != 0) {
			xedit_file_browser_widget_remove_filter (obj,
								 obj->
								 priv->
								 glob_filter_id);
		...
		if (obj->priv->glob_filter_id == 0)
			obj->priv->glob_filter_id =
			    xedit_file_browser_widget_add_filter (obj,
								  filter_glob,
								  NULL,
								  NULL);
	}
	...
	if (XEDIT_IS_FILE_BROWSER_STORE (model))
		xedit_file_browser_store_refilter (XEDIT_FILE_BROWSER_STORE
						   (model));
...
GtkWidget *
xedit_file_browser_widget_new (const gchar *data_dir)
{
	XeditFileBrowserWidget *obj =
	    g_object_new (XEDIT_TYPE_FILE_BROWSER_WIDGET, NULL);
	...
	xedit_file_browser_widget_show_bookmarks (obj);

	return GTK_WIDGET (obj);
}

void
xedit_file_browser_widget_show_bookmarks (XeditFileBrowserWidget * obj)
{
	...
	xedit_file_browser_view_set_model (obj->priv->treeview,
					   GTK_TREE_MODEL (obj->priv->
							   bookmarks_store));
}

static void
show_files_real (XeditFileBrowserWidget *obj,
...
void
xedit_file_browser_widget_show_files (XeditFileBrowserWidget * obj)
{
						     ...
						     gchar const *virtual_root)
{
	...
	if (result == XEDIT_FILE_BROWSER_STORE_RESULT_NO_CHANGE)
		show_files_real (obj, TRUE);
}

void
xedit_file_browser_widget_set_root (XeditFileBrowserWidget * obj,
				    gchar const *root, 
...
gulong
xedit_file_browser_widget_add_filter (XeditFileBrowserWidget * obj,
				      XeditFileBrowserWidgetFilterFunc func, 
				      gpointer user_data,
				      GDestroyNotify notify)
{
...
void
xedit_file_browser_widget_remove_filter (XeditFileBrowserWidget * obj,
					 gulong id)
{
	GSList *item;
	FilterFunc *func;
...
void 
xedit_file_browser_widget_set_filter_pattern (XeditFileBrowserWidget * obj,
                                              gchar const *pattern)
{
						  ...
						  GtkTreeIter * iter)
{
	...
	if (!xedit_file_browser_widget_get_first_selected (obj, iter)) {
		if (!xedit_file_browser_store_get_iter_virtual_root
		    (XEDIT_FILE_BROWSER_STORE (model), iter))
			return FALSE;
	}
...
static guint
xedit_file_browser_widget_get_num_selected_files_or_directories (XeditFileBrowserWidget *obj,
								 guint                  *files,
								 guint                  *dirs)
{
	GList *rows, *row;
...
static AsyncData *
async_data_new (XeditFileBrowserWidget *widget)
{
	AsyncData *ret;
...
static void
set_busy (XeditFileBrowserWidget *obj, gboolean busy)
{
#if !GTK_CHECK_VERSION (3, 16, 0)
	GdkCursor *cursor;
#endif
...
static void try_mount_volume (XeditFileBrowserWidget *widget, GVolume *volume);

static void
activate_mount (XeditFileBrowserWidget *widget,
		GVolume		       *volume,
		GMount		       *mount)
{
	GFile *root;
	...
	xedit_file_browser_widget_set_root (widget, uri, FALSE);
...
static void
try_activate_drive (XeditFileBrowserWidget *widget,
		    GDrive 		   *drive)
{
	GList *volumes;
	GVolume *volume;
...
static void
activate_drive (XeditFileBrowserWidget *obj,
		GtkTreeIter	       *iter)
{
	GDrive *drive;
	AsyncData *async;
...
static void 
try_mount_volume (XeditFileBrowserWidget *widget, 
		  GVolume 		 *volume)
{
...
static void
activate_volume (XeditFileBrowserWidget *obj,
		 GtkTreeIter	        *iter)
{
	GVolume *volume;
...
void 
xedit_file_browser_widget_refresh (XeditFileBrowserWidget *obj)
{
	GtkTreeModel *model =
	    gtk_tree_view_get_model (GTK_TREE_VIEW (obj->priv->treeview));

	if (XEDIT_IS_FILE_BROWSER_STORE (model))
		xedit_file_browser_store_refresh (XEDIT_FILE_BROWSER_STORE
						  (model));
	else if (XEDIT_IS_FILE_BOOKMARKS_STORE (model)) {
		g_hash_table_ref (obj->priv->bookmarks_hash);
		g_hash_table_destroy (obj->priv->bookmarks_hash);

		xedit_file_bookmarks_store_refresh
		    (XEDIT_FILE_BOOKMARKS_STORE (model));
	}
}

void 
xedit_file_browser_widget_history_back (XeditFileBrowserWidget *obj)
{
	if (obj->priv->locations) {
		if (obj->priv->current_location)
...
void
xedit_file_browser_widget_history_forward (XeditFileBrowserWidget *obj)
{
	if (obj->priv->locations)
...
static void
bookmark_open (XeditFileBrowserWidget *obj,
	       GtkTreeModel           *model,
	       GtkTreeIter            *iter)
{
	gchar *uri;
	...
	if (flags & XEDIT_FILE_BOOKMARKS_STORE_IS_DRIVE)
	{
		/* handle a drive node */
		xedit_file_browser_store_cancel_mount_operation (obj->priv->file_store);
		activate_drive (obj, iter);
		return;
	}
	else if (flags & XEDIT_FILE_BOOKMARKS_STORE_IS_VOLUME)
	{
	...
	uri =
	    xedit_file_bookmarks_store_get_uri
	    (XEDIT_FILE_BOOKMARKS_STORE (model), iter);
	...
	if (uri) {
		/* here we check if the bookmark is a mount point, or if it
		   ...
		   mount point anymore) */
		if ((flags & XEDIT_FILE_BOOKMARKS_STORE_IS_MOUNT) ||
		    (flags & XEDIT_FILE_BOOKMARKS_STORE_IS_REMOTE_BOOKMARK)) {
			xedit_file_browser_widget_set_root (obj,
							    uri,
							    FALSE);
		} else {
			xedit_file_browser_widget_set_root (obj,
...
static void
file_open  (XeditFileBrowserWidget *obj,
	    GtkTreeModel           *model,
	    GtkTreeIter            *iter)
{
	gchar *uri;
...
static gboolean
directory_open (XeditFileBrowserWidget *obj,
		GtkTreeModel           *model,
		GtkTreeIter            *iter)
{
...
static void
on_bookmark_activated (XeditFileBrowserView   *tree_view,
		       GtkTreeIter            *iter,
		       XeditFileBrowserWidget *obj)
{
...
static void
on_file_activated (XeditFileBrowserView   *tree_view, 
		   GtkTreeIter            *iter,
		   XeditFileBrowserWidget *obj)
{
	GtkTreeModel *model = gtk_tree_view_get_model (GTK_TREE_VIEW (tree_view));
...
static gboolean
virtual_root_is_root (XeditFileBrowserWidget * obj, 
                      XeditFileBrowserStore  * model)
{
	GtkTreeIter root;
	GtkTreeIter virtual_root;
	
	if (!xedit_file_browser_store_get_iter_root (model, &root))
		return TRUE;
	
	if (!xedit_file_browser_store_get_iter_virtual_root (model, &virtual_root))
		return TRUE;
	
	return xedit_file_browser_store_iter_equal (model, &root, &virtual_root);
}

static void
on_virtual_root_changed (XeditFileBrowserStore * model,
			 GParamSpec * param,
			 XeditFileBrowserWidget * obj)
{
	GtkTreeIter iter;
	...
	if (xedit_file_browser_store_get_iter_virtual_root (model, &iter)) {
		...
		if (xedit_file_browser_store_get_iter_root (model, &root)) {
			if (!obj->priv->changing_location) {
				...
				root_uri =
				    xedit_file_browser_store_get_root
				    (model);
	      ...
	      XeditFileBrowserWidget * obj)
{
	...
	} else if (XEDIT_IS_FILE_BROWSER_STORE (model)) {
...
static void
on_file_store_error (XeditFileBrowserStore * store, guint code,
		     gchar * message, XeditFileBrowserWidget * obj)
{
	g_signal_emit (obj, signals[ERROR], 0, code, message);
}

static void
on_treeview_error (XeditFileBrowserView * tree_view, guint code,
		   gchar * message, XeditFileBrowserWidget * obj)
{
	g_signal_emit (obj, signals[ERROR], 0, code, message);
}
...
static void
on_combo_changed (GtkComboBox * combo, XeditFileBrowserWidget * obj)
{
	GtkTreeIter iter;
	guint id;
	gchar * uri;
...
static gboolean
on_treeview_popup_menu (XeditFileBrowserView * treeview,
			XeditFileBrowserWidget * obj)
{
...
static gboolean
on_treeview_button_press_event (XeditFileBrowserView * treeview,
				GdkEventButton * event,
				XeditFileBrowserWidget * obj)
{
...
static gboolean
do_change_directory (XeditFileBrowserWidget * obj,
                     GdkEventKey            * event)
{
...
static gboolean
on_treeview_key_press_event (XeditFileBrowserView * treeview,
			     GdkEventKey * event,
			     XeditFileBrowserWidget * obj)
{
	guint modifiers;
	...
	if (!XEDIT_IS_FILE_BROWSER_STORE
	    (gtk_tree_view_get_model (GTK_TREE_VIEW (treeview))))
		return FALSE;
		      ...
		      XeditFileBrowserWidget * obj)
{
	...
	if (XEDIT_IS_FILE_BROWSER_STORE (model))
	{
		selected = xedit_file_browser_widget_get_num_selected_files_or_directories (obj,
											    &files,
											    &dirs);
	}
...
static gboolean
on_entry_filter_activate (XeditFileBrowserWidget * obj)
{
	gchar const *text;
                          ...
                          XeditFileBrowserWidget *obj)
{
	...
	uri = xedit_file_bookmarks_store_get_uri (obj->priv->bookmarks_store, &iter);
...
static void 
on_filter_mode_changed (XeditFileBrowserStore * model,
                        GParamSpec * param,
                        XeditFileBrowserWidget * obj)
{
	gint mode;
	...
	action = GTK_TOGGLE_ACTION (gtk_action_group_get_action (obj->priv->action_group, 
	                                                         "FilterHidden"));
	active = !(mode & XEDIT_FILE_BROWSER_STORE_FILTER_MODE_HIDE_HIDDEN);
	...
	action = GTK_TOGGLE_ACTION (gtk_action_group_get_action (obj->priv->action_group, 
	                                                         "FilterBinary"));
	active = !(mode & XEDIT_FILE_BROWSER_STORE_FILTER_MODE_HIDE_BINARY);
...
static void
on_action_directory_next (GtkAction * action, XeditFileBrowserWidget * obj)
{
	xedit_file_browser_widget_history_forward (obj);
}
...
on_action_directory_previous (GtkAction * action,
			      XeditFileBrowserWidget * obj)
{
	xedit_file_browser_widget_history_back (obj);
}
			...
			XeditFileBrowserWidget * obj)
{
	...
	xedit_file_browser_store_set_virtual_root_up (XEDIT_FILE_BROWSER_STORE (model));	
}

static void
on_action_directory_new (GtkAction * action, XeditFileBrowserWidget * obj)
{
	GtkTreeModel *model;
	GtkTreeIter parent;
	GtkTreeIter iter;
	...
	if (!xedit_file_browser_widget_get_selected_directory (obj, &parent))
		return;

	if (xedit_file_browser_store_new_directory
	    (XEDIT_FILE_BROWSER_STORE (model), &parent, &iter)) {
		xedit_file_browser_view_start_rename (obj->priv->treeview,
						      &iter);
	}
}
...
static void
on_action_file_open (GtkAction * action, XeditFileBrowserWidget * obj)
{
	GtkTreeModel *model;
	GtkTreeSelection *selection;
	GList *rows;
...
static void
on_action_file_new (GtkAction * action, XeditFileBrowserWidget * obj)
{
	GtkTreeModel *model;
	GtkTreeIter parent;
	GtkTreeIter iter;
	...
	if (xedit_file_browser_store_new_file
	    (XEDIT_FILE_BROWSER_STORE (model), &parent, &iter)) {
		xedit_file_browser_view_start_rename (obj->priv->treeview,
						      &iter);
	}
...
static void
on_action_file_rename (GtkAction * action, XeditFileBrowserWidget * obj)
{
	rename_selected_file (obj);
}

static void
on_action_file_delete (GtkAction * action, XeditFileBrowserWidget * obj)
{
	delete_selected_files (obj, FALSE);
}

static void
on_action_file_move_to_trash (GtkAction * action, XeditFileBrowserWidget * obj)
{
	delete_selected_files (obj, TRUE);
}
...
on_action_directory_refresh (GtkAction * action,
			     XeditFileBrowserWidget * obj)
{
	xedit_file_browser_widget_refresh (obj);
}

static void
on_action_directory_open (GtkAction * action, XeditFileBrowserWidget * obj)
{
	GtkTreeModel *model;
	GtkTreeSelection *selection;
	GList *rows;
	...
	if (!directory_opened) {
		if (xedit_file_browser_widget_get_selected_directory (obj, &iter))
...
static void
on_action_filter_hidden (GtkAction * action, XeditFileBrowserWidget * obj)
{
...
static void
on_action_filter_binary (GtkAction * action, XeditFileBrowserWidget * obj)
{
	update_filter_mode (obj, 
	                    action, 
	                    XEDIT_FILE_BROWSER_STORE_FILTER_MODE_HIDE_BINARY);
...
static void
on_action_bookmark_open (GtkAction * action, XeditFileBrowserWidget * obj)
{
	GtkTreeModel *model;
	GtkTreeSelection *selection;
	GtkTreeIter iter;
