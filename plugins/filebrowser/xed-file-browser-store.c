			               ...
			               GParamSpec   *pspec)
{
	...
	{
		case PROP_FILTER_MODE:
			xed_file_browser_store_set_filter_mode (obj,
...
xed_file_browser_store_init (XedFileBrowserStore * obj)
{
	...
	obj->priv->column_types[XED_FILE_BROWSER_STORE_COLUMN_URI] =
	    G_TYPE_STRING;
	obj->priv->column_types[XED_FILE_BROWSER_STORE_COLUMN_NAME] =
	    G_TYPE_STRING;
	obj->priv->column_types[XED_FILE_BROWSER_STORE_COLUMN_FLAGS] =
	    G_TYPE_UINT;
	obj->priv->column_types[XED_FILE_BROWSER_STORE_COLUMN_ICON] =
	    GDK_TYPE_PIXBUF;
	obj->priv->column_types[XED_FILE_BROWSER_STORE_COLUMN_EMBLEM] =
	    GDK_TYPE_PIXBUF;
...
static gboolean
node_in_tree (XedFileBrowserStore * model, FileBrowserNode * node)
{
...
{
	g_return_val_if_fail (XED_IS_FILE_BROWSER_STORE (tree_model),
			      (GtkTreeModelFlags) 0);
...
static GType
xed_file_browser_store_get_column_type (GtkTreeModel * tree_model, gint idx)
{
	g_return_val_if_fail (XED_IS_FILE_BROWSER_STORE (tree_model),
			      G_TYPE_INVALID);
	g_return_val_if_fail (idx < XED_FILE_BROWSER_STORE_COLUMN_NUM &&
...
/* Private */
static void
model_begin_loading (XedFileBrowserStore * model, FileBrowserNode * node)
{
	GtkTreeIter iter;
...
static void
model_end_loading (XedFileBrowserStore * model, FileBrowserNode * node)
{
	GtkTreeIter iter;
			      ...
			      FileBrowserNode * node)
{
	...
	if (FILTER_HIDDEN (model->priv->filter_mode) &&
		...
		if (!model->priv->
			...
			node->flags |=
			    XED_FILE_BROWSER_STORE_FLAG_IS_FILTERED;
	}
}
...
static void
model_resort_node (XedFileBrowserStore * model, FileBrowserNode * node)
{
	FileBrowserNodeDir *dir;
	GSList *item;
	FileBrowserNode *child;
		     ...
		     GtkTreePath ** path)
{
	...
	{
		if (in_tree)
			tmppath = xed_file_browser_store_get_path_real (model,
								       node);
		else
 ...
 **/
static void
model_clear (XedFileBrowserStore * model, gboolean free_nodes)
{
	GtkTreePath *path;
	FileBrowserNodeDir *dir;
...
static void
model_check_dummy (XedFileBrowserStore * model, FileBrowserNode * node)
{
...
static void
model_add_node (XedFileBrowserStore * model, FileBrowserNode * child,
		FileBrowserNode * parent)
{
...
static void
model_fill (XedFileBrowserStore * model, FileBrowserNode * node,
	    GtkTreePath ** path)
{
	gboolean free_path = FALSE;
	GtkTreeIter iter = {0,};
...
model_root_mounted (XedFileBrowserStore * model, gchar const * virtual_root)
{
...
static void
handle_root_error (XedFileBrowserStore * model, GError *error)
{
	FileBrowserNode * root;
...
static XedFileBrowserStoreResult
model_mount_root (XedFileBrowserStore * model, gchar const * virtual_root)
{
	GFileInfo * info;
	GError * error = NULL;
	MountInfo * mount_info;
...
{
	XedFileBrowserStore *obj =
	    XED_FILE_BROWSER_STORE (g_object_new
				      (XED_TYPE_FILE_BROWSER_STORE,
				       NULL));
...
XedFileBrowserStoreResult
xed_file_browser_store_set_virtual_root_from_string
    (XedFileBrowserStore * model, gchar const *root) {
	GFile *file;
...
XedFileBrowserStoreResult
xed_file_browser_store_set_virtual_root_top (XedFileBrowserStore *
					       model)
{
	g_return_val_if_fail (XED_IS_FILE_BROWSER_STORE (model),
			      XED_FILE_BROWSER_STORE_RESULT_NO_CHANGE);
...
XedFileBrowserStoreResult
xed_file_browser_store_set_virtual_root_up (XedFileBrowserStore *
					      model)
{
	g_return_val_if_fail (XED_IS_FILE_BROWSER_STORE (model),
			      XED_FILE_BROWSER_STORE_RESULT_NO_CHANGE);
...
gboolean
xed_file_browser_store_get_iter_virtual_root (XedFileBrowserStore *
...
XedFileBrowserStoreResult
xed_file_browser_store_set_root_and_virtual_root (XedFileBrowserStore *
						    model,
						    gchar const *root,
				   ...
				   gchar const *root)
{
	g_return_val_if_fail (XED_IS_FILE_BROWSER_STORE (model),
			      XED_FILE_BROWSER_STORE_RESULT_NO_CHANGE);
	return xed_file_browser_store_set_root_and_virtual_root (model,
								   root,
								   NULL);
}
	...
	if (g_file_move (node->file, file, G_FILE_COPY_NONE, NULL, NULL, NULL, &err)) {
		...
		if (err) {
			if (error != NULL) {
				*error =
				    g_error_new_literal
				    (xed_file_browser_store_error_quark (),
				     XED_FILE_BROWSER_ERROR_RENAME,
				     err->message);
			}
