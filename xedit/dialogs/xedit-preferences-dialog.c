...
/*
 * Modified by the xedit Team, 2001-2003. See the AUTHORS file for a 
...
#include "xedit-preferences-dialog.h"
...
/*
 * xedit-preferences dialog is a singleton since we don't
 * want two dialogs showing an inconsistent state of the
 * preferences.
 * When xedit_show_preferences_dialog is called and there
...
#define XEDIT_PREFERENCES_DIALOG_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE ((object), \
						     XEDIT_TYPE_PREFERENCES_DIALOG, \
						     XeditPreferencesDialogPrivate))

struct _XeditPreferencesDialogPrivate
{
	GtkWidget	*notebook;
...
G_DEFINE_TYPE(XeditPreferencesDialog, xedit_preferences_dialog, GTK_TYPE_DIALOG)


static void 
xedit_preferences_dialog_class_init (XeditPreferencesDialogClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	g_type_class_add_private (object_class, sizeof (XeditPreferencesDialogPrivate));
}
...
tabs_width_spinbutton_value_changed (GtkSpinButton          *spin_button,
				     XeditPreferencesDialog *dlg)
{
	xedit_debug (DEBUG_PREFS);

	g_return_if_fail (spin_button == GTK_SPIN_BUTTON (dlg->priv->tabs_width_spinbutton));

	xedit_prefs_manager_set_tabs_size (gtk_spin_button_get_value_as_int (spin_button));
}
				   ...
				   XeditPreferencesDialog *dlg)
{
	...
	xedit_prefs_manager_set_insert_spaces (gtk_toggle_button_get_active (button));
}
				 ...
				 XeditPreferencesDialog *dlg)
{
	...
	xedit_prefs_manager_set_auto_indent (gtk_toggle_button_get_active (button));
}
			       ...
			       XeditPreferencesDialog *dlg)
{
	...
	if (gtk_toggle_button_get_active (button))
	{
		...
		xedit_prefs_manager_set_auto_save (TRUE);
	}
	else	
	{
		gtk_widget_set_sensitive (dlg->priv->auto_save_spinbutton, FALSE);
		xedit_prefs_manager_set_auto_save (FALSE);
	}
}
				 ...
				 XeditPreferencesDialog *dlg)
{
	...
	xedit_prefs_manager_set_create_backup_copy (gtk_toggle_button_get_active (button));
}
...
static void
setup_editor_page (XeditPreferencesDialog *dlg)
{
	gboolean auto_save;
	...
	gtk_spin_button_set_value (GTK_SPIN_BUTTON (dlg->priv->tabs_width_spinbutton),
				   (guint) xedit_prefs_manager_get_tabs_size ());
	...
	auto_save = xedit_prefs_manager_get_auto_save ();
	gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (dlg->priv->auto_save_checkbutton),
				      auto_save);

	auto_save_interval = xedit_prefs_manager_get_auto_save_interval ();
					  ...
					  XeditPreferencesDialog *dlg)
{
	...
	xedit_prefs_manager_set_display_line_numbers (gtk_toggle_button_get_active (button));
}
					    ...
					    XeditPreferencesDialog *dlg)
{
	...
	xedit_prefs_manager_set_highlight_current_line (gtk_toggle_button_get_active (button));
}
			       ...
			       XeditPreferencesDialog *dlg)
{
	...
	{
		xedit_prefs_manager_set_wrap_mode (GTK_WRAP_NONE);
	...
	else
	{
		...
		if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (dlg->priv->split_checkbutton)))
		{
			...
			xedit_prefs_manager_set_wrap_mode (GTK_WRAP_WORD);
		}
		else
		{
			split_button_state = FALSE;
			
			xedit_prefs_manager_set_wrap_mode (GTK_WRAP_CHAR);
		}
	}
}
				  ...
				  XeditPreferencesDialog *dlg)
{
	...
	xedit_prefs_manager_set_display_right_margin (active);
						...
						XeditPreferencesDialog *dlg)
{
	...
	xedit_prefs_manager_set_right_margin_position (value);
}

static void
setup_view_page (XeditPreferencesDialog *dlg)
{
	...
	display_right_margin = xedit_prefs_manager_get_display_right_margin ();
	...
	gtk_spin_button_set_value (
		...
		(guint)CLAMP (xedit_prefs_manager_get_right_margin_position (), 1, 160));
	...
	wrap_mode_can_set = xedit_prefs_manager_wrap_mode_can_set ();
				       ...
				       XeditPreferencesDialog *dlg)
{
	...
	if (gtk_toggle_button_get_active (button))
	{
		...
		xedit_prefs_manager_set_use_default_font (TRUE);
	}
	else
	{
		gtk_widget_set_sensitive (dlg->priv->font_hbox, 
					  xedit_prefs_manager_editor_font_can_set ());
		xedit_prefs_manager_set_use_default_font (FALSE);
	}
}
			     ...
			     XeditPreferencesDialog *dlg)
{
	...
	xedit_prefs_manager_set_editor_font (font_name);
}

static void
setup_font_colors_page_font_section (XeditPreferencesDialog *dlg)
{
	...
	xedit_utils_set_atk_relation (dlg->priv->font_button,
				      dlg->priv->default_font_checkbutton,
				      ATK_RELATION_CONTROLLED_BY);
	xedit_utils_set_atk_relation (dlg->priv->default_font_checkbutton,
				      dlg->priv->font_button, 
				      ATK_RELATION_CONTROLLER_FOR);	

	editor_font = xedit_prefs_manager_get_system_font ();
	...
	use_default_font = xedit_prefs_manager_get_use_default_font ();
...
static void
set_buttons_sensisitivity_according_to_scheme (XeditPreferencesDialog *dlg,
					       const gchar            *scheme_id)
{
	gboolean editable;
	
	editable = (scheme_id != NULL) && 
	           _xedit_style_scheme_manager_scheme_is_xedit_user_scheme (
		      ...
		      XeditPreferencesDialog *dlg)
{
	...
	xedit_prefs_manager_set_source_style_scheme (id);
...
ensure_color_scheme_id (const gchar *id)
{
	...
	GtkSourceStyleSchemeManager *manager = xedit_get_style_scheme_manager ();

	if (id == NULL)
	{
		gchar *pref_id;

		pref_id = xedit_prefs_manager_get_source_style_scheme ();
 ...
 * xedit_style_scheme_manager_get_default_scheme. If this one returns NULL
 * use the first available scheme as default */
static const gchar *
populate_color_scheme_list (XeditPreferencesDialog *dlg, const gchar *def_id)
{
	GSList *schemes;
	GSList *l;
	...
	schemes = xedit_style_scheme_manager_list_schemes_sorted (xedit_get_style_scheme_manager ());
	l = schemes;
	while (l != NULL)
	{
				...
				XeditPreferencesDialog *dlg)
{
	...
	scheme_id = _xedit_style_scheme_manager_install_scheme (
					xedit_get_style_scheme_manager (),
					filename);
	g_free (filename);
	...
	if (scheme_id == NULL)
	{
		xedit_warning (GTK_WINDOW (dlg),
			       _("The selected color scheme cannot be installed."));

		return;
	}

	xedit_prefs_manager_set_source_style_scheme (scheme_id);
			...
			XeditPreferencesDialog *dlg)
{
	...
	xedit_dialog_add_button (GTK_DIALOG (chooser), 
				 _("A_dd Scheme"),
				 GTK_STOCK_ADD,
				 GTK_RESPONSE_ACCEPT);
			  ...
			  XeditPreferencesDialog *dlg)
{
					     ...
					     &iter))
	{
		...
		if (!_xedit_style_scheme_manager_uninstall_scheme (xedit_get_style_scheme_manager (), id))
		{
			xedit_warning (GTK_WINDOW (dlg),
		...
		{	
			...
			if (real_new_id != NULL)
				xedit_prefs_manager_set_source_style_scheme (real_new_id);
		}
...
static void
setup_font_colors_page_style_scheme_section (XeditPreferencesDialog *dlg)
{
	GtkCellRenderer *renderer;
...
static void
setup_font_colors_page (XeditPreferencesDialog *dlg)
{
...
static void
setup_plugins_page (XeditPreferencesDialog *dlg)
{
	GtkWidget *page_content;

	xedit_debug (DEBUG_PREFS);

	page_content = xedit_plugin_manager_new ();
...
static void
xedit_preferences_dialog_init (XeditPreferencesDialog *dlg)
{
	...
	dlg->priv = XEDIT_PREFERENCES_DIALOG_GET_PRIVATE (dlg);
	...
	gtk_window_set_title (GTK_WINDOW (dlg), _("Xedit Preferences"));
	...
	file = xedit_dirs_get_ui_file ("xedit-preferences-dialog.ui");
...
void
xedit_show_preferences_dialog (XeditWindow *parent)
{
	xedit_debug (DEBUG_PREFS);

	g_return_if_fail (XEDIT_IS_WINDOW (parent));

	if (preferences_dialog == NULL)
	{
		preferences_dialog = GTK_WIDGET (g_object_new (XEDIT_TYPE_PREFERENCES_DIALOG, NULL));
