	         ...
	         const gchar  *uri_for_display)
{
	...
	case G_IO_ERROR_NOT_SUPPORTED:
		{
			...
			if ((scheme_string != NULL) && g_utf8_validate (scheme_string, -1, NULL))
			{
				...
				*message_details = g_strdup_printf (_("xedit cannot handle %s locations."),
								   scheme_markup);
				g_free (scheme_markup);
			}
			else
			{
				*message_details = g_strdup (_("xedit cannot handle this location."));
			}	
	...
	case G_IO_ERROR_HOST_NOT_FOUND:
		 ...
		 */
		{
			...
			if (xedit_utils_decode_uri (uri, NULL, NULL, &hn, NULL, NULL))
			{
				if (hn != NULL)
				{
					gchar *host_markup;
					...
					host_name = xedit_utils_make_valid_utf8 (hn);
					g_free (hn);
	           ...
	           const gchar  *uri_for_display)
{
	...
	switch (code)
	{
	case XEDIT_DOCUMENT_ERROR_TOO_BIG:
	     ...
	     const gchar  *uri_for_display)
{
	...
	{
		ret = parse_xedit_error (error->code,
					 error_message,
					 message_details,
					 uri,
					 uri_for_display);
...
GtkWidget *
xedit_unrecoverable_reverting_error_message_area_new (const gchar  *uri,
						      const GError *error)
{
	...
	g_return_val_if_fail ((error->domain == XEDIT_DOCUMENT_ERROR) || 
			      (error->domain == G_IO_ERROR), NULL);

	full_formatted_uri = xedit_utils_uri_for_display (uri);

	/* Truncate the URI so it doesn't get insanely wide. Note that even
	 * though the dialog uses wrapped text, if the URI doesn't contain
	 * white space then the text-wrapping code is too stupid to wrap it.
	 */
	temp_uri_for_display = xedit_utils_str_middle_truncate (full_formatted_uri, 
	...
	{
		message_details = g_strdup (_("xedit cannot find the file. "
...
create_combo_box (GtkWidget *message_area, GtkWidget *vbox)
{
	...
	menu = xedit_encodings_combo_box_new (TRUE);
	g_object_set_data (G_OBJECT (message_area), 
			   "xedit-message-area-encoding-menu", 
			   menu);
...
GtkWidget *
xedit_io_loading_error_message_area_new (const gchar         *uri,
					 ...
					 const GError        *error)
{
	...
	if (encoding != NULL)
		encoding_name = xedit_encoding_to_string (encoding);
	else
	...
	else if (is_gio_error (error, G_IO_ERROR_PERMISSION_DENIED))
	{
		message_details = g_strdup (_("You do not have the permissions necessary to open the file."));
	         ...
	         (error->domain == XEDIT_DOCUMENT_ERROR &&
	         error->code == XEDIT_DOCUMENT_ERROR_ENCODING_AUTO_DETECTION_FAILED))
	{
		message_details = g_strconcat (_("xedit has not been able to detect "
...
GtkWidget *
xedit_conversion_error_while_saving_message_area_new (
						const gchar         *uri,
						const XeditEncoding *encoding,
				    		const GError        *error)
{
...
xedit_conversion_error_message_area_get_encoding (GtkWidget *message_area)
{
	...
	return xedit_encodings_combo_box_get_selected_encoding
					(XEDIT_ENCODINGS_COMBO_BOX (menu));
}

GtkWidget *
xedit_file_already_open_warning_message_area_new (const gchar *uri)
{
	GtkWidget *message_area;
	GtkWidget *hbox_content;
	GtkWidget *image;
	...
	primary_text = g_strdup_printf (_("This file (%s) is already open in another xedit window."), uri_for_display);
	g_free (uri_for_display);
	...
	secondary_text = _("xedit opened this instance of the file in a non-editable way. "
...
GtkWidget *
xedit_externally_modified_saving_error_message_area_new (
						const gchar  *uri,
						const GError *error)
{
	GtkWidget *message_area;
	...
	g_return_val_if_fail (uri != NULL, NULL);
	g_return_val_if_fail (error != NULL, NULL);
	g_return_val_if_fail (error->domain == XEDIT_DOCUMENT_ERROR, NULL);
	g_return_val_if_fail (error->code == XEDIT_DOCUMENT_ERROR_EXTERNALLY_MODIFIED, NULL);
...
GtkWidget *
xedit_no_backup_saving_error_message_area_new (const gchar  *uri,
					       const GError *error)
{
	GtkWidget *message_area;
	GtkWidget *hbox_content;
	...
	if (xedit_prefs_manager_get_create_backup_copy ())
	...
	secondary_text = _("xedit could not back up the old copy of the file before saving the new one. "
...
GtkWidget *
xedit_unrecoverable_saving_error_message_area_new (const gchar  *uri,
						   const GError *error)
{
	gchar *error_message = NULL;
	gchar *message_details = NULL;
	...
	if (is_gio_error (error, G_IO_ERROR_NOT_SUPPORTED))
	{
		...
		if ((scheme_string != NULL) && g_utf8_validate (scheme_string, -1, NULL))
		{
			...
			/* Translators: %s is a URI scheme (like for example http:, ftp:, etc.) */
			message_details = g_strdup_printf (_("xedit cannot handle %s locations in write mode. "
		...
		{
			message_details = g_strdup (_("xedit cannot handle this location in write mode. "
						      "Please check that you typed the "
						      "location correctly and try again."));
		}
	...
	else if (is_gio_error (error, G_IO_ERROR_FILENAME_TOO_LONG))
	{
		message_details = g_strdup (_("The disk where you are trying to save the file has "
		 ...
		 error->code == XEDIT_DOCUMENT_ERROR_TOO_BIG)
	{
		message_details = g_strdup (_("The disk where you are trying to save the file has "
...
GtkWidget *
xedit_externally_modified_message_area_new (const gchar *uri,
