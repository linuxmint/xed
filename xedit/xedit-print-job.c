 ...
 *
 * $Id: xedit-print.c 6022 2007-12-09 14:38:57Z pborelli $
 */
...
#define XEDIT_PRINT_JOB_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE ((object), \
					    XEDIT_TYPE_PRINT_JOB, \
					    XeditPrintJobPrivate))

struct _XeditPrintJobPrivate
{
	XeditView                *view;
	XeditDocument            *doc;
...
G_DEFINE_TYPE (XeditPrintJob, xedit_print_job, G_TYPE_OBJECT)

static void
set_view (XeditPrintJob *job, XeditView *view)
{
	job->priv->view = view;
	job->priv->doc = XEDIT_DOCUMENT (gtk_text_view_get_buffer (GTK_TEXT_VIEW (view)));
}

static void 
xedit_print_job_get_property (GObject    *object,
			      guint       prop_id,
			      GValue     *value,
			      GParamSpec *pspec)
{
	XeditPrintJob *job = XEDIT_PRINT_JOB (object);
...
static void 
xedit_print_job_set_property (GObject      *object,
			      guint         prop_id,
			      const GValue *value,
			      GParamSpec   *pspec)
{
...
static void
xedit_print_job_finalize (GObject *object)
{
	XeditPrintJob *job = XEDIT_PRINT_JOB (object);
	...
	G_OBJECT_CLASS (xedit_print_job_parent_class)->finalize (object);
}

static void 
xedit_print_job_class_init (XeditPrintJobClass *klass)
{
	GObjectClass *object_class;

	object_class = G_OBJECT_CLASS (klass);

	object_class->get_property = xedit_print_job_get_property;
	object_class->set_property = xedit_print_job_set_property;
	object_class->finalize = xedit_print_job_finalize;
	...
	print_job_signals[PRINTING] =
		g_signal_new ("printing",
			      ...
			      G_SIGNAL_RUN_LAST,
			      G_STRUCT_OFFSET (XeditPrintJobClass, printing),
			      NULL, NULL,
	...
	print_job_signals[SHOW_PREVIEW] =
		g_signal_new ("show-preview",
			      G_OBJECT_CLASS_TYPE (object_class),
			      G_SIGNAL_RUN_LAST,
			      G_STRUCT_OFFSET (XeditPrintJobClass, show_preview),
			      NULL, NULL,
	...
	print_job_signals[DONE] =
		g_signal_new ("done",
			      G_OBJECT_CLASS_TYPE (object_class),
			      G_SIGNAL_RUN_LAST,
			      G_STRUCT_OFFSET (XeditPrintJobClass, done),
			      NULL, NULL,
			      xedit_marshal_VOID__UINT_POINTER,
			      G_TYPE_NONE,
			      2,
			      G_TYPE_UINT,
			      G_TYPE_POINTER);
			      			      
	g_type_class_add_private (object_class, sizeof (XeditPrintJobPrivate));
}
...
{
	if (xedit_prefs_manager_print_font_body_can_set ())
	{
		gchar *font;

		font = xedit_prefs_manager_get_default_print_font_body ();
	...
	if (xedit_prefs_manager_print_font_header_can_set ())
	{
		gchar *font;

		font = xedit_prefs_manager_get_default_print_font_header ();
	...
	if (xedit_prefs_manager_print_font_numbers_can_set ())
	{
		gchar *font;

		font = xedit_prefs_manager_get_default_print_font_numbers ();
			 ...
			 XeditPrintJob     *job)
{
	...
	file = xedit_dirs_get_ui_file ("xedit-print-preferences.ui");
	...
	/* Line numbers */
	line_numbers =  xedit_prefs_manager_get_print_line_numbers ();
	can_set = xedit_prefs_manager_print_line_numbers_can_set ();
	...
	/* Text wrapping */
	wrap_mode = xedit_prefs_manager_get_print_wrap_mode ();
	...
	can_set = xedit_prefs_manager_print_wrap_mode_can_set ();
...
custom_widget_apply_cb (GtkPrintOperation *operation,
			...
			XeditPrintJob     *job)
{
	xedit_prefs_manager_set_print_syntax_hl (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (job->priv->syntax_checkbutton)));

	xedit_prefs_manager_set_print_header (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (job->priv->page_header_checkbutton)));
	...
	{
		xedit_prefs_manager_set_print_line_numbers (0);
	}

	if (!gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (job->priv->text_wrapping_checkbutton)))
	{
		xedit_prefs_manager_set_print_wrap_mode (GTK_WRAP_NONE);
	}
	else
	{
		if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (job->priv->do_not_split_checkbutton)))
		{
			xedit_prefs_manager_set_print_wrap_mode (GTK_WRAP_WORD);
		}
		else
		{
			xedit_prefs_manager_set_print_wrap_mode (GTK_WRAP_CHAR);
		}	
	}

	xedit_prefs_manager_set_print_font_body (gtk_font_button_get_font_name (GTK_FONT_BUTTON (job->priv->body_fontbutton)));
	xedit_prefs_manager_set_print_font_header (gtk_font_button_get_font_name (GTK_FONT_BUTTON (job->priv->headers_fontbutton)));
	xedit_prefs_manager_set_print_font_numbers (gtk_font_button_get_font_name (GTK_FONT_BUTTON (job->priv->numbers_fontbutton)));
}

static void
create_compositor (XeditPrintJob *job)
{
	...
	print_font_body = xedit_prefs_manager_get_print_font_body ();
	print_font_header = xedit_prefs_manager_get_print_font_header ();
	print_font_numbers = xedit_prefs_manager_get_print_font_numbers ();
	...
	job->priv->compositor = GTK_SOURCE_PRINT_COMPOSITOR (
					...
					g_object_new (GTK_TYPE_SOURCE_PRINT_COMPOSITOR,
						     ...
						     "highlight-syntax", gtk_source_buffer_get_highlight_syntax (GTK_SOURCE_BUFFER (job->priv->doc)) &&
					   				 xedit_prefs_manager_get_print_syntax_hl (),
						     "wrap-mode", xedit_prefs_manager_get_print_wrap_mode (),
						     "print-line-numbers", xedit_prefs_manager_get_print_line_numbers (),
						     "print-header", xedit_prefs_manager_get_print_header (),
						     "print-footer", FALSE,
	...
	if (xedit_prefs_manager_get_print_header ())
	{
		gchar *doc_name;
		gchar *name_to_display;
		gchar *left;

		doc_name = xedit_document_get_uri_for_display (job->priv->doc);
		name_to_display = xedit_utils_str_middle_truncate (doc_name, 60);
	        ...
	        XeditPrintJob     *job)
{
	...
	job->priv->status = XEDIT_PRINT_JOB_STATUS_PAGINATING;
...
preview_cb (GtkPrintOperation        *op,
	    ...
	    XeditPrintJob            *job)
{
	job->priv->preview = xedit_print_preview_new (op, gtk_preview, context);
	      ...
	      XeditPrintJob     *job)
{
	...
	if (!job->priv->is_preview)
	{
		...
		job->priv->status = XEDIT_PRINT_JOB_STATUS_DRAWING;
...
{
	GError *error = NULL;
	XeditPrintJobResult print_result;

	switch (result) 
	{
		case GTK_PRINT_OPERATION_RESULT_CANCEL:
			print_result = XEDIT_PRINT_JOB_RESULT_CANCEL;
			break;

		case GTK_PRINT_OPERATION_RESULT_APPLY:
			print_result = XEDIT_PRINT_JOB_RESULT_OK;
			break;

		case GTK_PRINT_OPERATION_RESULT_ERROR:
			print_result = XEDIT_PRINT_JOB_RESULT_ERROR;
...
/* Note that xedit_print_job_print can can only be called once on a given XeditPrintJob */
GtkPrintOperationResult	 
xedit_print_job_print (XeditPrintJob            *job,
		       ...
		       GError                  **error)
{
	...
	job_name = xedit_document_get_short_name_for_display (priv->doc);
...
static void
xedit_print_job_init (XeditPrintJob *job)
{
	job->priv = XEDIT_PRINT_JOB_GET_PRIVATE (job);
	
	job->priv->status = XEDIT_PRINT_JOB_STATUS_INIT;
	
	job->priv->status_string = g_strdup (_("Preparing..."));
}

XeditPrintJob *
xedit_print_job_new (XeditView *view)
{
	XeditPrintJob *job;
	
	g_return_val_if_fail (XEDIT_IS_VIEW (view), NULL);
	
	job = XEDIT_PRINT_JOB (g_object_new (XEDIT_TYPE_PRINT_JOB,
					     "view", view,
					      NULL));
...
void
xedit_print_job_cancel (XeditPrintJob *job)
{
	g_return_if_fail (XEDIT_IS_PRINT_JOB (job));

	gtk_print_operation_cancel (job->priv->operation);
}

const gchar *
xedit_print_job_get_status_string (XeditPrintJob *job)
{
	g_return_val_if_fail (XEDIT_IS_PRINT_JOB (job), NULL);
...
gdouble
xedit_print_job_get_progress (XeditPrintJob *job)
{
	g_return_val_if_fail (XEDIT_IS_PRINT_JOB (job), 0.0);

	return job->priv->progress;
}

GtkPrintSettings *
xedit_print_job_get_print_settings (XeditPrintJob *job)
{
	g_return_val_if_fail (XEDIT_IS_PRINT_JOB (job), NULL);
...
GtkPageSetup *
xedit_print_job_get_page_setup (XeditPrintJob *job)
{
	g_return_val_if_fail (XEDIT_IS_PRINT_JOB (job), NULL);
