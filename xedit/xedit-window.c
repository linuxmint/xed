...
#include "xedit-status-combo-box.h"

#define LANGUAGE_NONE (const gchar *)"LangNone"
#define XEDIT_UIFILE "xedit-ui.xml"
#define TAB_WIDTH_DATA "XeditWindowTabWidthData"
#define LANGUAGE_DATA "XeditWindowLanguageData"
#define FULLSCREEN_ANIMATION_SPEED 4

#define XEDIT_WINDOW_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE ((object),\
					 XEDIT_TYPE_WINDOW,                    \
					 XeditWindowPrivate))
...
G_DEFINE_TYPE(XeditWindow, xedit_window, GTK_TYPE_WINDOW)

static void	recent_manager_changed	(GtkRecentManager *manager,
					 XeditWindow *window);

static void
xedit_window_get_property (GObject    *object,
			   guint       prop_id,
			   GValue     *value,
			   GParamSpec *pspec)
{
	XeditWindow *window = XEDIT_WINDOW (object);

	switch (prop_id)
	{
		case PROP_STATE:
			g_value_set_enum (value,
					  xedit_window_get_state (window));
			break;			
		default:
...
static void
save_panes_state (XeditWindow *window)
{
	gint pane_page;

	xedit_debug (DEBUG_WINDOW);

	if (xedit_prefs_manager_window_size_can_set ())
		xedit_prefs_manager_set_window_size (window->priv->width,
						     window->priv->height);

	if (xedit_prefs_manager_window_state_can_set ())
		xedit_prefs_manager_set_window_state (window->priv->window_state);
	...
	pane_page = _xedit_panel_get_active_item_id (XEDIT_PANEL (window->priv->side_panel));
	if (pane_page != 0 &&
	    xedit_prefs_manager_side_panel_active_page_can_set ())
		xedit_prefs_manager_set_side_panel_active_page (pane_page);
	...
	pane_page = _xedit_panel_get_active_item_id (XEDIT_PANEL (window->priv->bottom_panel));
	if (pane_page != 0 &&
	    xedit_prefs_manager_bottom_panel_active_page_can_set ())
		xedit_prefs_manager_set_bottom_panel_active_page (pane_page);
}

static void
xedit_window_dispose (GObject *object)
{
	XeditWindow *window;

	xedit_debug (DEBUG_WINDOW);

	window = XEDIT_WINDOW (object);
	 ...
	 */
	xedit_plugins_engine_garbage_collect (xedit_plugins_engine_get_default ());
	...
	if (!window->priv->dispose_has_run)
	{
		...
		xedit_plugins_engine_deactivate_plugins (xedit_plugins_engine_get_default (),
					                  window);
	...
	G_OBJECT_CLASS (xedit_window_parent_class)->dispose (object);
}

static void
xedit_window_finalize (GObject *object)
{
	XeditWindow *window; 
	...
	G_OBJECT_CLASS (xedit_window_parent_class)->finalize (object);
}

static gboolean
xedit_window_window_state_event (GtkWidget           *widget,
				 GdkEventWindowState *event)
{
	XeditWindow *window = XEDIT_WINDOW (widget);
...
static gboolean 
xedit_window_configure_event (GtkWidget         *widget,
			      GdkEventConfigure *event)
{
	...
	return GTK_WIDGET_CLASS (xedit_window_parent_class)->configure_event (widget, event);
}
 ...
 */
static gboolean
xedit_window_key_press_event (GtkWidget   *widget,
			      GdkEventKey *event)
{
	...
	if (grand_parent_class == NULL)
		grand_parent_class = g_type_class_peek_parent (xedit_window_parent_class);
...
static void
xedit_window_tab_removed (XeditWindow *window,
			  XeditTab    *tab) 
{
	xedit_plugins_engine_garbage_collect (xedit_plugins_engine_get_default ());
}
...
static void
xedit_window_class_init (XeditWindowClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);
	GtkWidgetClass *widget_class = GTK_WIDGET_CLASS (klass);

	klass->tab_removed = xedit_window_tab_removed;

	object_class->dispose = xedit_window_dispose;
	object_class->finalize = xedit_window_finalize;
	object_class->get_property = xedit_window_get_property;

	widget_class->window_state_event = xedit_window_window_state_event;
	widget_class->configure_event = xedit_window_configure_event;
	widget_class->key_press_event = xedit_window_key_press_event;

	signals[TAB_ADDED] =
		g_signal_new ("tab_added",
			      G_OBJECT_CLASS_TYPE (object_class),
			      G_SIGNAL_RUN_FIRST,
			      G_STRUCT_OFFSET (XeditWindowClass, tab_added),
			      NULL, NULL,
	...
	signals[TAB_REMOVED] =
		g_signal_new ("tab_removed",
			      G_OBJECT_CLASS_TYPE (object_class),
			      G_SIGNAL_RUN_FIRST,
			      G_STRUCT_OFFSET (XeditWindowClass, tab_removed),
			      NULL, NULL,
			      g_cclosure_marshal_VOID__OBJECT,
			      G_TYPE_NONE,
			      1,
	...
	signals[TABS_REORDERED] =
		g_signal_new ("tabs_reordered",
			      G_OBJECT_CLASS_TYPE (object_class),
			      G_SIGNAL_RUN_FIRST,
			      G_STRUCT_OFFSET (XeditWindowClass, tabs_reordered),
			      NULL, NULL,
	...
	signals[ACTIVE_TAB_CHANGED] =
		g_signal_new ("active_tab_changed",
			      G_OBJECT_CLASS_TYPE (object_class),
			      G_SIGNAL_RUN_FIRST,
			      G_STRUCT_OFFSET (XeditWindowClass, active_tab_changed),
			      NULL, NULL,
			      g_cclosure_marshal_VOID__OBJECT,
			      G_TYPE_NONE,
			      1,
	...
	signals[ACTIVE_TAB_STATE_CHANGED] =
		g_signal_new ("active_tab_state_changed",
			      G_OBJECT_CLASS_TYPE (object_class),
			      G_SIGNAL_RUN_FIRST,
			      G_STRUCT_OFFSET (XeditWindowClass, active_tab_state_changed),
			      NULL, NULL,
			      g_cclosure_marshal_VOID__VOID,
			      G_TYPE_NONE,
			      0);			      			      
	...
	g_object_class_install_property (object_class,
					 ...
					 g_param_spec_flags ("state",
							     ...
							     XEDIT_WINDOW_STATE_NORMAL,
							     G_PARAM_READABLE |
							     G_PARAM_STATIC_STRINGS));

	g_type_class_add_private (object_class, sizeof(XeditWindowPrivate));
}
...
static void
apply_toolbar_style (XeditWindow *window,
		     GtkWidget *toolbar)
{
	switch (window->priv->toolbar_style)
	{
		case XEDIT_TOOLBAR_SYSTEM:
			xedit_debug_message (DEBUG_WINDOW, "XEDIT: SYSTEM");
			gtk_toolbar_unset_style (
					GTK_TOOLBAR (toolbar));
			break;

		case XEDIT_TOOLBAR_ICONS:
			xedit_debug_message (DEBUG_WINDOW, "XEDIT: ICONS");
		...
		case XEDIT_TOOLBAR_ICONS_AND_TEXT:
			xedit_debug_message (DEBUG_WINDOW, "XEDIT: ICONS_AND_TEXT");
			gtk_toolbar_set_style (
					GTK_TOOLBAR (toolbar),
					GTK_TOOLBAR_BOTH);
			break;
		...
		case XEDIT_TOOLBAR_ICONS_BOTH_HORIZ:
			xedit_debug_message (DEBUG_WINDOW, "XEDIT: ICONS_BOTH_HORIZ");
			gtk_toolbar_set_style (
					GTK_TOOLBAR (toolbar),
...
static gboolean
set_toolbar_style (XeditWindow *window,
		   XeditWindow *origin)
{
	gboolean visible;
	XeditToolbarSetting style;
	GtkAction *action;
	
	if (origin == NULL)
		visible = xedit_prefs_manager_get_toolbar_visible ();
	else
	...
	/* Set style */
	if (origin == NULL)
		style = xedit_prefs_manager_get_toolbar_buttons_style ();
	else
...
static void
update_next_prev_doc_sensitivity (XeditWindow *window,
				  XeditTab    *tab)
{
	gint	     tab_number;
...
static void
update_next_prev_doc_sensitivity_per_window (XeditWindow *window)
{
	XeditTab  *tab;
	GtkAction *action;
			     ...
			     XeditWindow      *window)
{
	...
	if (window->priv->active_tab != NULL)
	{
		...
		state = xedit_tab_get_state (window->priv->active_tab);
...
static void
set_paste_sensitivity_according_to_clipboard (XeditWindow  *window,
...
static void
set_sensitivity_according_to_tab (XeditWindow *window,
				  XeditTab    *tab)
{
	XeditDocument *doc;
	XeditView     *view;
	...
	b = xedit_document_get_can_search_again (doc);
	action = gtk_action_group_get_action (window->priv->action_group,
					      "SearchFindNext");
	gtk_action_set_sensitive (action,
				  (state_normal ||
				   state == XEDIT_TAB_STATE_EXTERNALLY_MODIFIED_NOTIFICATION) && b);
	...
	xedit_plugins_engine_update_plugins_ui (xedit_plugins_engine_get_default (),
						 window);
}
		  ...
		  XeditWindow     *window)
{
	...
	xedit_document_set_language (doc, lang);
}
			   ...
			   XeditWindow       *window)
{
	...
	if (section_action == NULL)
	{
		...
		section_name = xedit_utils_escape_underscores (section, -1);
	...
	escaped_lang_name = xedit_utils_escape_underscores (lang_name, -1);
...
static void
create_languages_menu (XeditWindow *window)
{
	...
	languages = xedit_language_manager_list_languages_sorted (
						xedit_get_language_manager (),
						FALSE);
...
static void
update_languages_menu (XeditWindow *window)
{
	XeditDocument *doc;
	GList *actions;
	GList *l;
...
void
_xedit_recent_add (XeditWindow *window,
		   const gchar *uri,
		   const gchar *mime)
{
	GtkRecentManager *recent_manager;
...
void
_xedit_recent_remove (XeditWindow *window,
		      const gchar *uri)
{
	GtkRecentManager *recent_manager;
		  ...
		  XeditWindow *window)
{
	...
	if (xedit_commands_load_uris (window, uris, NULL, 0) != 1)
	{
		_xedit_recent_remove (window, uri);
	}
...
static void	update_recent_files_menu (XeditWindow *window);
 ...
 */
static void
update_recent_files_menu (XeditWindow *window)
{
	XeditWindowPrivate *p = window->priv;
	GtkRecentManager *recent_manager;
	gint max_recents;
	...
	max_recents = xedit_prefs_manager_get_max_recents ();
	...
	for (l = items; l != NULL; l = l->next)
	{
		...
		if (!gtk_recent_info_has_group (info, "xedit"))
			continue;
	...
	for (l = filtered_items; l != NULL; l = l->next)
	{
		...
		escaped = xedit_utils_escape_underscores (display_name, -1);
		if (i >= 10)
		...
		uri = xedit_utils_uri_for_display (gtk_recent_info_get_uri (info));
			    ...
			    XeditWindow *window)
{
	...
	if (xedit_prefs_manager_toolbar_visible_can_set ())
		xedit_prefs_manager_set_toolbar_visible (visible);
...
static GtkWidget *
setup_toolbar_open_button (XeditWindow *window,
			   GtkWidget *toolbar)
{
	GtkRecentManager *recent_manager;
	...
	gtk_recent_filter_add_group (filter, "xedit");
...
static void
create_menu_bar_and_toolbar (XeditWindow *window, 
			     GtkWidget   *main_box)
{
	...
	action_group = gtk_action_group_new ("XeditWindowAlwaysSensitiveActions");
	gtk_action_group_set_translation_domain (action_group, NULL);
	gtk_action_group_add_actions (action_group,
				      xedit_always_sensitive_menu_entries,
				      G_N_ELEMENTS (xedit_always_sensitive_menu_entries),
				      window);
	gtk_action_group_add_toggle_actions (action_group,
					     xedit_always_sensitive_toggle_menu_entries,
					     G_N_ELEMENTS (xedit_always_sensitive_toggle_menu_entries),
					     window);
	...
	action_group = gtk_action_group_new ("XeditWindowActions");
	gtk_action_group_set_translation_domain (action_group, NULL);
	gtk_action_group_add_actions (action_group,
				      xedit_menu_entries,
				      G_N_ELEMENTS (xedit_menu_entries),
				      window);
	gtk_ui_manager_insert_action_group (manager, action_group, 0);
	g_object_unref (action_group);
	...
	action_group = gtk_action_group_new ("XeditQuitWindowActions");
	gtk_action_group_set_translation_domain (action_group, NULL);
	gtk_action_group_add_actions (action_group,
				      xedit_quit_menu_entries,
				      G_N_ELEMENTS (xedit_quit_menu_entries),
				      window);
	...
	action_group = gtk_action_group_new ("XeditCloseWindowActions");
	gtk_action_group_set_translation_domain (action_group, NULL);
	gtk_action_group_add_actions (action_group,
	                              xedit_close_menu_entries,
	                              G_N_ELEMENTS (xedit_close_menu_entries),
	                              window);
	...
	action_group = gtk_action_group_new ("XeditWindowPanesActions");
	gtk_action_group_set_translation_domain (action_group, NULL);
	gtk_action_group_add_toggle_actions (action_group,
					     xedit_panes_toggle_menu_entries,
					     G_N_ELEMENTS (xedit_panes_toggle_menu_entries),
					     window);
	...
	ui_file = xedit_dirs_get_ui_file (XEDIT_UIFILE);
...
static gchar *
get_menu_tip_for_tab (XeditTab *tab)
{
	XeditDocument *doc;
	gchar *uri;
	gchar *ruri;
...
static void
update_documents_list_menu (XeditWindow *window)
{
	XeditWindowPrivate *p = window->priv;
	GList *actions, *l;
	gint n, i;
	...
	for (i = 0; i < n; i++)
	{
		...
		tab_name = _xedit_tab_get_name (XEDIT_TAB (tab));
		name = xedit_utils_escape_underscores (tab_name, -1);
		tip =  get_menu_tip_for_tab (XEDIT_TAB (tab));
...
static gboolean
set_statusbar_style (XeditWindow *window,
		     XeditWindow *origin)
{
	GtkAction *action;
	...
	if (origin == NULL)
		visible = xedit_prefs_manager_get_statusbar_visible ();
	else
			      ...
			      XeditWindow *window)
{
	...
	if (xedit_prefs_manager_statusbar_visible_can_set ())
		xedit_prefs_manager_set_statusbar_visible (visible);
...
static void
tab_width_combo_changed (XeditStatusComboBox *combo,
			 GtkMenuItem         *item,
			 XeditWindow         *window)
{
	XeditView *view;
...
static void
language_combo_changed (XeditStatusComboBox *combo,
			GtkMenuItem         *item,
			XeditWindow         *window)
{
	XeditDocument *doc;
	...
	xedit_document_set_language (doc, language);
...
static void
fill_tab_width_combo (XeditWindow *window)
{
	...
	XeditStatusComboBox *combo = XEDIT_STATUS_COMBO_BOX (window->priv->tab_width_combo);
	guint i = 0;
	GtkWidget *item;
	...
	while (defs[i].label != NULL)
	{
		...
		xedit_status_combo_box_add_item (combo,
						 GTK_MENU_ITEM (item),
						 defs[i].label);
	...
	xedit_status_combo_box_add_item (combo, GTK_MENU_ITEM (item), NULL);
	gtk_widget_show (item);
...
static void
fill_language_combo (XeditWindow *window)
{
	...
	languages = xedit_language_manager_list_languages_sorted (manager, FALSE);
	...
	xedit_status_combo_box_add_item (XEDIT_STATUS_COMBO_BOX (window->priv->language_combo),
...
static void
create_statusbar (XeditWindow *window, 
		  GtkWidget   *main_box)
{
	xedit_debug (DEBUG_WINDOW);

	window->priv->statusbar = xedit_statusbar_new ();
	...
	window->priv->tab_width_combo = xedit_status_combo_box_new (_("Tab Width"));
	...
	window->priv->language_combo = xedit_status_combo_box_new (NULL);
...
static XeditWindow *
clone_window (XeditWindow *origin)
{
	XeditWindow *window;
	GdkScreen *screen;
	XeditApp  *app;
	...
	panel_page = _xedit_panel_get_active_item_id (XEDIT_PANEL (origin->priv->side_panel));
	_xedit_panel_set_active_item_by_id (XEDIT_PANEL (window->priv->side_panel),
					    panel_page);

	panel_page = _xedit_panel_get_active_item_id (XEDIT_PANEL (origin->priv->bottom_panel));
	_xedit_panel_set_active_item_by_id (XEDIT_PANEL (window->priv->bottom_panel),
					    panel_page);
				  ...
				  XeditWindow   *window)
{
	...
	xedit_debug (DEBUG_WINDOW);
 	...
 	if (buffer != GTK_TEXT_BUFFER (xedit_window_get_active_document (window)))
 		return;
...
update_overwrite_mode_statusbar (GtkTextView *view, 
				 XeditWindow *window)
{
	if (view != GTK_TEXT_VIEW (xedit_window_get_active_view (window)))
		return;
...
set_title (XeditWindow *window)
{
	...
	if (window->priv->active_tab == NULL)
	{
		gtk_window_set_title (GTK_WINDOW (window), "Xedit");
		return;
	}

	doc = xedit_tab_get_document (window->priv->active_tab);
	...
	if (len > MAX_TITLE_LENGTH)
	{
		...
		tmp = xedit_utils_str_middle_truncate (name,
						       MAX_TITLE_LENGTH);
		g_free (name);
		name = tmp;
	}
...
static void
set_tab_width_item_blocked (XeditWindow *window,
			    GtkMenuItem *item)
{
	...
	xedit_status_combo_box_set_item (XEDIT_STATUS_COMBO_BOX (window->priv->tab_width_combo),
					 item);
...
spaces_instead_of_tabs_changed (GObject     *object,
		 		...
		 		XeditWindow *window)
{
	XeditView *view = XEDIT_VIEW (object);
	gboolean active = gtk_source_view_get_insert_spaces_instead_of_tabs (
			GTK_SOURCE_VIEW (view));
	GList *children = xedit_status_combo_box_get_items (
			XEDIT_STATUS_COMBO_BOX (window->priv->tab_width_combo));
		   ...
		   XeditWindow *window)
{
	...
	items = xedit_status_combo_box_get_items (combo);
	...
	for (item = items; item; item = item->next)
	{
		...
		if (GTK_IS_SEPARATOR_MENU_ITEM (item->next->data))
		{
			if (!found)
			{
				...
				xedit_status_combo_box_set_item_text (combo, 
		  ...
		  XeditWindow *window)
{
	GList *items;
	GList *item;
	XeditStatusComboBox *combo = XEDIT_STATUS_COMBO_BOX (window->priv->language_combo);
	...
	for (item = items; item; item = item->next)
	{
		    					       ...
		    					       new_id) == 0))
		{
			...
			xedit_status_combo_box_set_item (XEDIT_STATUS_COMBO_BOX (window->priv->language_combo),
					 		 GTK_MENU_ITEM (item->data));
		      ...
		      XeditWindow     *window)
{
	...
	tab = XEDIT_TAB (gtk_notebook_get_nth_page (book, page_num));
	...
	if (window->priv->active_tab)
	{
		...
		{
			g_signal_handler_disconnect (xedit_tab_get_view (window->priv->active_tab), 
	...
	/* sync the statusbar */
	update_cursor_position_statusbar (GTK_TEXT_BUFFER (xedit_tab_get_document (tab)),
					  window);
	xedit_statusbar_set_overwrite (XEDIT_STATUSBAR (window->priv->statusbar),
	...
	window->priv->language_changed_id = g_signal_connect (xedit_tab_get_document (tab),
							      "notify::language",
	...
	language_changed (G_OBJECT (xedit_tab_get_document (tab)), NULL, window);
...
set_sensitivity_according_to_window_state (XeditWindow *window)
{
	...
	gtk_action_group_set_sensitive (window->priv->quit_action_group,
				  !(window->priv->state & XEDIT_WINDOW_STATE_SAVING) &&
	...
	action = gtk_action_group_get_action (window->priv->always_sensitive_action_group,
					      "FileNew");
	gtk_action_set_sensitive (action, 
				  !(window->priv->state & XEDIT_WINDOW_STATE_SAVING_SESSION));
	...
	xedit_notebook_set_close_buttons_sensitive (XEDIT_NOTEBOOK (window->priv->notebook),
						    !(window->priv->state & XEDIT_WINDOW_STATE_SAVING_SESSION));
						    
	xedit_notebook_set_tab_drag_and_drop_enabled (XEDIT_NOTEBOOK (window->priv->notebook),
						      !(window->priv->state & XEDIT_WINDOW_STATE_SAVING_SESSION));

	if ((window->priv->state & XEDIT_WINDOW_STATE_SAVING_SESSION) != 0)
	{
		   ...
		   XeditWindow *window)
{
	...
	switch (ts)
	{
		...
		case XEDIT_TAB_STATE_REVERTING:
			window->priv->state |= XEDIT_WINDOW_STATE_LOADING;
			break;
		...
		case XEDIT_TAB_STATE_GENERIC_ERROR:
			window->priv->state |= XEDIT_WINDOW_STATE_ERROR;
...
static void
update_window_state (XeditWindow *window)
{
	XeditWindowState old_ws;
	gint old_num_of_errors;
	
	xedit_debug_message (DEBUG_WINDOW, "Old state: %x", window->priv->state);
	
	old_ws = window->priv->state;
	old_num_of_errors = window->priv->num_tabs_with_error;
	
	window->priv->state = old_ws & XEDIT_WINDOW_STATE_SAVING_SESSION;
	...
	xedit_debug_message (DEBUG_WINDOW, "New state: %x", window->priv->state);		
		
	if (old_ws != window->priv->state)
	{
		set_sensitivity_according_to_window_state (window);

		xedit_statusbar_set_window_state (XEDIT_STATUSBAR (window->priv->statusbar),
						  window->priv->state,
						  window->priv->num_tabs_with_error);
	   ...
	   XeditWindow *window)
{
	...
	tab_name = _xedit_tab_get_name (tab);
	escaped_name = xedit_utils_escape_underscores (tab_name, -1);
...
get_drop_window (GtkWidget *widget)
{
	...
	g_return_val_if_fail (XEDIT_IS_WINDOW (target_window), NULL);

	if ((XEDIT_WINDOW(target_window)->priv->state & XEDIT_WINDOW_STATE_SAVING_SESSION) != 0)
		return NULL;
	
	return XEDIT_WINDOW (target_window);
}

static void
load_uris_from_drop (XeditWindow  *window,
		     gchar       **uri_list)
{
	GSList *uris = NULL;
	gint i;
	...
	xedit_commands_load_uris (window,
				  uris,
				  NULL,
				  0);
...
/* Handle drops on the XeditWindow */
static void
		       ...
		       gpointer          data)
{
	...
	{
		uri_list = xedit_utils_drop_get_uris(selection_data);
		load_uris_from_drop (window, uri_list);
		g_strfreev (uri_list);
	}
}

/* Handle drops on the XeditView */
static void
...
static void
fullscreen_controls_show (XeditWindow *window)
{
	GdkScreen *screen;
...
{
	XeditWindow *window = XEDIT_WINDOW (data);
	GdkScreen *screen;
	GdkRectangle fs_rect;
	gint x, y;
...
static void
show_hide_fullscreen_toolbar (XeditWindow *window,
			      gboolean     show,
			      gint         height)
{
...
static void
fullscreen_controls_build (XeditWindow *window)
{
	XeditWindowPrivate *priv = window->priv;
	GtkWidget *toolbar;
	GtkWidget *toolbar_recent_menu;
	GtkAction *action;
...
static void
can_search_again (XeditDocument *doc,
		  GParamSpec    *pspec,
		  XeditWindow   *window)
{
	gboolean sensitive;
	...
	if (doc != xedit_window_get_active_document (window))
		return;

	sensitive = xedit_document_get_can_search_again (doc);
...
static void
selection_changed (XeditDocument *doc,
		   GParamSpec    *pspec,
		   XeditWindow   *window)
{
	XeditTab *tab;
...
static void
sync_languages_menu (XeditDocument *doc,
		     GParamSpec    *pspec,
		     XeditWindow   *window)
{
	update_languages_menu (window);
...
static void
readonly_changed (XeditDocument *doc,
		  GParamSpec    *pspec,
		  XeditWindow   *window)
{
...
static void
editable_changed (XeditView  *view,
                  GParamSpec  *arg1,
                  XeditWindow *window)
{
	xedit_plugins_engine_update_plugins_ui (xedit_plugins_engine_get_default (),
...
static void
update_sensitivity_according_to_open_tabs (XeditWindow *window)
{
	GtkAction *action;
...
static void
notebook_tab_added (XeditNotebook *notebook,
		    XeditTab      *tab,
		    XeditWindow   *window)
{
	XeditView *view;
	...
	g_return_if_fail ((window->priv->state & XEDIT_WINDOW_STATE_SAVING_SESSION) == 0);
...
static void
notebook_tab_removed (XeditNotebook *notebook,
		      XeditTab      *tab,
		      XeditWindow   *window)
{
	XeditView     *view;
	...
	if (window->priv->tab_width_id && tab == xedit_window_get_active_tab (window))
	{
		g_signal_handler_disconnect (view, window->priv->tab_width_id);
		window->priv->tab_width_id = 0;
	}
	
	if (window->priv->spaces_instead_of_tabs_id && tab == xedit_window_get_active_tab (window))
	{
		g_signal_handler_disconnect (view, window->priv->spaces_instead_of_tabs_id);
		window->priv->spaces_instead_of_tabs_id = 0;
	}
	
	if (window->priv->language_changed_id && tab == xedit_window_get_active_tab (window))
	{
...
static void
notebook_tabs_reordered (XeditNotebook *notebook,
			 XeditWindow   *window)
{
	update_documents_list_menu (window);
	update_next_prev_doc_sensitivity_per_window (window);
...
static void
notebook_tab_detached (XeditNotebook *notebook,
		       XeditTab      *tab,
		       XeditWindow   *window)
{
	XeditWindow *new_window;
	
	new_window = clone_window (window);
		
	xedit_notebook_move_tab (notebook,
				 XEDIT_NOTEBOOK (_xedit_window_get_notebook (new_window)),
				 tab, 0);
...
// CHECK do we need this?
	...
	else
	{
		...
		tab = GTK_WIDGET (xedit_window_get_active_tab (window));
		g_return_val_if_fail (tab != NULL, FALSE);

		tab_label = gtk_notebook_get_tab_label (notebook, tab);

		gtk_menu_popup (GTK_MENU (menu), NULL, NULL,
				xedit_utils_menu_position_under_widget, tab_label,
		     ...
		     XeditWindow *window)
{
	...
	if (XEDIT_IS_NOTEBOOK (gtk_window_get_focus (GTK_WINDOW (window))))
	{
			 ...
			 XeditWindow *window)
{
	...
	xedit_debug_message (DEBUG_WINDOW,
			       ...
			       XeditWindow *window)
{
	...
	if (xedit_prefs_manager_side_pane_visible_can_set ())
		xedit_prefs_manager_set_side_pane_visible (visible);
	...
	if (!visible && window->priv->active_tab != NULL)
		gtk_widget_grab_focus (GTK_WIDGET (
				xedit_tab_get_view (XEDIT_TAB (window->priv->active_tab))));
}

static void
create_side_panel (XeditWindow *window)
{
	GtkWidget *documents_panel;

	xedit_debug (DEBUG_WINDOW);

	window->priv->side_panel = xedit_panel_new (GTK_ORIENTATION_VERTICAL);
	...
	documents_panel = xedit_documents_panel_new (window);
	xedit_panel_add_item_with_stock_icon (XEDIT_PANEL (window->priv->side_panel),
					      documents_panel,
					      _("Documents"),
					      GTK_STOCK_FILE);
}
...
static void
bottom_panel_visibility_changed (XeditPanel  *bottom_panel,
				 XeditWindow *window)
{
	gboolean visible;
	GtkAction *action;
	...
	if (xedit_prefs_manager_bottom_panel_visible_can_set ())
		xedit_prefs_manager_set_bottom_panel_visible (visible);
...
static void
bottom_panel_item_removed (XeditPanel  *panel,
			   GtkWidget   *item,
			   XeditWindow *window)
{
	if (xedit_panel_get_n_items (panel) == 0)
	{
		GtkAction *action;
...
static void
create_bottom_panel (XeditWindow *window) 
{
	xedit_debug (DEBUG_WINDOW);

	window->priv->bottom_panel = xedit_panel_new (GTK_ORIENTATION_HORIZONTAL);
...
static void
init_panels_visibility (XeditWindow *window)
{
	gint active_page;
	...
	/* side pane */
	active_page = xedit_prefs_manager_get_side_panel_active_page ();
	_xedit_panel_set_active_item_by_id (XEDIT_PANEL (window->priv->side_panel),
					    active_page);

	if (xedit_prefs_manager_get_side_pane_visible ())
	{
		gtk_widget_show (window->priv->side_panel);
	}

	/* bottom pane, it can be empty */
	if (xedit_panel_get_n_items (XEDIT_PANEL (window->priv->bottom_panel)) > 0)
	{
		active_page = xedit_prefs_manager_get_bottom_panel_active_page ();
		_xedit_panel_set_active_item_by_id (XEDIT_PANEL (window->priv->bottom_panel),
						    active_page);

		if (xedit_prefs_manager_get_bottom_panel_visible ())
		{
			gtk_widget_show (window->priv->bottom_panel);
		}
	}
...
static void
check_window_is_active (XeditWindow *window,
			GParamSpec *property,
			gpointer useless)
{
...
static void
connect_notebook_signals (XeditWindow *window,
			  GtkWidget   *notebook)
{
...
static void
add_notebook (XeditWindow *window,
	      GtkWidget   *notebook)
{
...
static void
xedit_window_init (XeditWindow *window)
{
	GtkWidget *main_box;
	GtkTargetList *tl;
	...
	window->priv = XEDIT_WINDOW_GET_PRIVATE (window);
	window->priv->active_tab = NULL;
	window->priv->num_tabs = 0;
	window->priv->removing_tabs = FALSE;
	window->priv->state = XEDIT_WINDOW_STATE_NORMAL;
	window->priv->dispose_has_run = FALSE;
	window->priv->fullscreen_controls = NULL;
	window->priv->fullscreen_animation_timeout_id = 0;

	window->priv->message_bus = xedit_message_bus_new ();
	...
	/* Add the main area */
	xedit_debug_message (DEBUG_WINDOW, "Add main area");		
	...
	xedit_debug_message (DEBUG_WINDOW, "Create xedit notebook");
	window->priv->notebook = xedit_notebook_new ();
	...
	window->priv->side_panel_size = xedit_prefs_manager_get_side_panel_size ();
	window->priv->bottom_panel_size = xedit_prefs_manager_get_bottom_panel_size ();
	...
	xedit_debug_message (DEBUG_WINDOW, "Update plugins ui");
	
	xedit_plugins_engine_activate_plugins (xedit_plugins_engine_get_default (),
					        window);
	...
	xedit_debug_message (DEBUG_WINDOW, "END");
}
 ...
 *
 * Gets the active #XeditView.
 *
 * Returns: the active #XeditView
 */
XeditView *
xedit_window_get_active_view (XeditWindow *window)
{
	XeditView *view;
	...
	view = xedit_tab_get_view (XEDIT_TAB (window->priv->active_tab));
 ...
 * @window: a #XeditWindow
 *
 * Gets the active #XeditDocument.
 * 
 * Returns: the active #XeditDocument
 */
XeditDocument *
xedit_window_get_active_document (XeditWindow *window)
{
	XeditView *view;
	...
	return XEDIT_DOCUMENT (gtk_text_view_get_buffer (GTK_TEXT_VIEW (view)));
}

GtkWidget *
_xedit_window_get_notebook (XeditWindow *window)
{
	g_return_val_if_fail (XEDIT_IS_WINDOW (window), NULL);
 ...
 * @window: a #XeditWindow
 * @jump_to: %TRUE to set the new #XeditTab as active
 *
 * Creates a new #XeditTab and adds the new tab to the #XeditNotebook.
 * In case @jump_to is %TRUE the #XeditNotebook switches to that new #XeditTab.
 *
 * Returns: a new #XeditTab
 */
XeditTab *
xedit_window_create_tab (XeditWindow *window,
			 gboolean     jump_to)
{
	XeditTab *tab;
	...
	xedit_notebook_add_tab (XEDIT_NOTEBOOK (window->priv->notebook),
				tab,
				-1,
				jump_to);
 ...
 * @encoding: a #XeditEncoding
 * @line_pos: the line position to visualize
 * @create: %TRUE to create a new document in case @uri does exist
 * @jump_to: %TRUE to set the new #XeditTab as active
 *
 * Creates a new #XeditTab loading the document specified by @uri.
 * In case @jump_to is %TRUE the #XeditNotebook swithes to that new #XeditTab.
 ...
 */
XeditTab *
xedit_window_create_tab_from_uri (XeditWindow         *window,
				  const gchar         *uri,
				  ...
				  gboolean             jump_to)
{
	...
	tab = _xedit_tab_new_from_uri (uri,
				       encoding,
				       line_pos,
				       create);	
	if (tab == NULL)
 ...
 * @window: a XeditWindow
 *
 * Gets the active #XeditTab in the @window.
 *
 * Returns: the active #XeditTab in the @window.
 */
XeditTab *
xedit_window_get_active_tab (XeditWindow *window)
{
	g_return_val_if_fail (XEDIT_IS_WINDOW (window), NULL);
	
	return (window->priv->active_tab == NULL) ? 
				NULL : XEDIT_TAB (window->priv->active_tab);
}

static void
add_document (XeditTab *tab, GList **res)
{
	XeditDocument *doc;
 ...
 *
 * Returns: (element-type Xedit.Document) (transfer container): a newly
 * allocated list with all the documents in the window
 */
GList *
xedit_window_get_documents (XeditWindow *window)
{
	GList *res = NULL;
...
static void
add_view (XeditTab *tab, GList **res)
{
	XeditView *view;
 ...
 *
 * Returns: (element-type Xedit.View) (transfer container): a newly allocated
 * list with all the views in the window
 */
GList *
xedit_window_get_views (XeditWindow *window)
{
	GList *res = NULL;
 ...
 * @window: a #XeditWindow
 * @tab: the #XeditTab to close
 *
 * Closes the @tab.
 */
void
xedit_window_close_tab (XeditWindow *window,
			XeditTab    *tab)
{
	g_return_if_fail (XEDIT_IS_WINDOW (window));
	g_return_if_fail (XEDIT_IS_TAB (tab));
	g_return_if_fail ((xedit_tab_get_state (tab) != XEDIT_TAB_STATE_SAVING) &&
			  (xedit_tab_get_state (tab) != XEDIT_TAB_STATE_SHOWING_PRINT_PREVIEW));
	
	xedit_notebook_remove_tab (XEDIT_NOTEBOOK (window->priv->notebook),
				   tab);
}
 ...
 */
void
xedit_window_close_all_tabs (XeditWindow *window)
{
	g_return_if_fail (XEDIT_IS_WINDOW (window));
	g_return_if_fail (!(window->priv->state & XEDIT_WINDOW_STATE_SAVING) &&
			  !(window->priv->state & XEDIT_WINDOW_STATE_SAVING_SESSION));

	window->priv->removing_tabs = TRUE;

	xedit_notebook_remove_all_tabs (XEDIT_NOTEBOOK (window->priv->notebook));
 ...
 * @window: a #XeditWindow
 * @tabs: (element-type Xedit.Tab): a list of #XeditTab
 *
 * Closes all tabs specified by @tabs.
 */
void
xedit_window_close_tabs (XeditWindow *window,
			 const GList *tabs)
{
	g_return_if_fail (XEDIT_IS_WINDOW (window));
	g_return_if_fail (!(window->priv->state & XEDIT_WINDOW_STATE_SAVING) &&
...
XeditWindow *
_xedit_window_move_tab_to_new_window (XeditWindow *window,
				      XeditTab    *tab)
{
	XeditWindow *new_window;
	...
	xedit_notebook_move_tab (XEDIT_NOTEBOOK (window->priv->notebook),
				 XEDIT_NOTEBOOK (new_window->priv->notebook),
				 tab,
				 -1);
 ...
 */
void
xedit_window_set_active_tab (XeditWindow *window,
			     XeditTab    *tab)
{
	gint page_num;
 ...
 */
GtkWindowGroup *
xedit_window_get_group (XeditWindow *window)
{
	g_return_val_if_fail (XEDIT_IS_WINDOW (window), NULL);
...
gboolean
_xedit_window_is_removing_tabs (XeditWindow *window)
{
 ...
 */
GtkUIManager *
xedit_window_get_ui_manager (XeditWindow *window)
{
	g_return_val_if_fail (XEDIT_IS_WINDOW (window), NULL);
 ...
 * @window: a #XeditWindow
 *
 * Gets the side #XeditPanel of the @window.
 *
 * Returns: the side #XeditPanel.
 */
XeditPanel *
xedit_window_get_side_panel (XeditWindow *window)
{
	g_return_val_if_fail (XEDIT_IS_WINDOW (window), NULL);

	return XEDIT_PANEL (window->priv->side_panel);
}
 ...
 * @window: a #XeditWindow
 *
 * Gets the bottom #XeditPanel of the @window.
 *
 * Returns: the bottom #XeditPanel.
 */
XeditPanel *
xedit_window_get_bottom_panel (XeditWindow *window)
{
	g_return_val_if_fail (XEDIT_IS_WINDOW (window), NULL);

	return XEDIT_PANEL (window->priv->bottom_panel);
}
 ...
 * @window: a #XeditWindow
 *
 * Gets the #XeditStatusbar of the @window.
 *
 * Returns: the #XeditStatusbar of the @window.
 */
GtkWidget *
xedit_window_get_statusbar (XeditWindow *window)
{
 ...
 *
 * Returns: the current #XeditWindowState of the @window.
 */
XeditWindowState
xedit_window_get_state (XeditWindow *window)
{
	g_return_val_if_fail (XEDIT_IS_WINDOW (window), XEDIT_WINDOW_STATE_NORMAL);

	return window->priv->state;
}

GFile *
_xedit_window_get_default_location (XeditWindow *window)
{
	g_return_val_if_fail (XEDIT_IS_WINDOW (window), NULL);
...
void
_xedit_window_set_default_location (XeditWindow *window,
				    GFile       *location)
{
	GFile *dir;
 ...
 *
 * Returns: (element-type Xedit.Document) (transfer container): a list of
 * #XeditDocument that need to be saved before closing the window
 */
GList *
xedit_window_get_unsaved_documents (XeditWindow *window)
{
...
void 
_xedit_window_set_saving_session_state (XeditWindow *window,
					gboolean     saving_session)
{
	XeditWindowState old_state;
...
void
_xedit_window_fullscreen (XeditWindow *window)
{
	g_return_if_fail (XEDIT_IS_WINDOW (window));
...
void
_xedit_window_unfullscreen (XeditWindow *window)
{
	gboolean visible;
	GtkAction *action;
...
gboolean
_xedit_window_is_fullscreen (XeditWindow *window)
{
	g_return_val_if_fail (XEDIT_IS_WINDOW (window), FALSE);
 ...
 * @location: a #GFile
 *
 * Gets the #XeditTab that matches with the given @location.
 *
 * Returns: the #XeditTab that matches with the given @location.
 */
XeditTab *
xedit_window_get_tab_from_location (XeditWindow *window,
				    GFile       *location)
{
	GList *tabs;
	GList *l;
	...
	for (l = tabs; l != NULL; l = g_list_next (l))
	{
		...
		t = XEDIT_TAB (l->data);
		d = xedit_tab_get_document (t);

		f = xedit_document_get_location (d);
 ...
 * @window: a #XeditWindow
 *
 * Gets the #XeditMessageBus associated with @window. The returned reference
 * is owned by the window and should not be unreffed.
 *
 * Return value: (transfer none): the #XeditMessageBus associated with @window
 */
XeditMessageBus	*
xedit_window_get_message_bus (XeditWindow *window)
{
	g_return_val_if_fail (XEDIT_IS_WINDOW (window), NULL);
 ...
 * @window: a #XeditWindow
 * @uri: the uri to get the #XeditTab
 *
 * Gets the #XeditTab that matches @uri.
 *
 * Returns: the #XeditTab associated with @uri.
 *
 * Deprecated: 2.24: Use xedit_window_get_tab_from_location() instead.
 */
XeditTab *
xedit_window_get_tab_from_uri (XeditWindow *window,
			       const gchar *uri)
{
	GFile *f;
	XeditTab *tab;
	...
	tab = xedit_window_get_tab_from_location (window, f);
	g_object_unref (f);
