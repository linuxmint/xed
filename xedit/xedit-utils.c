...
/**
 * xedit_utils_uris_has_file_scheme
 *
 * Returns: %TRUE if @uri is a file: uri and is not a chained uri
 */
gboolean
xedit_utils_uri_has_file_scheme (const gchar *uri)
{
	GFile *gfile;
	gboolean res;
...
gboolean
xedit_utils_uri_has_writable_scheme (const gchar *uri)
{
	GFile *gfile;
	gchar *scheme;
	...
	writable_schemes = xedit_prefs_manager_get_writable_vfs_schemes ();
...
void
xedit_utils_menu_position_under_widget (GtkMenu  *menu,
					gint     *x,
					gint     *y,
					gboolean *push_in,
					gpointer  user_data)
...
void
xedit_utils_menu_position_under_tree_view (GtkMenu  *menu,
					   gint     *x,
					   gint     *y,
					   gboolean *push_in,
					   gpointer  user_data)
{
	...
	else
	{
		...
		xedit_utils_menu_position_under_widget (menu,
							x, y, push_in,
							tree);
	}
}
...
gchar * 
xedit_gdk_color_to_string (GdkColor color)
{
...
GtkWidget *
xedit_gtk_button_new_with_stock_icon (const gchar *label,
...
GtkWidget *
xedit_dialog_add_button (GtkDialog   *dialog,
			 const gchar *text,
			 const gchar *stock_id,
			 gint         response_id)
{
	...
	button = xedit_gtk_button_new_with_stock_icon (text, stock_id);
 ...
 */
void
xedit_utils_set_atk_name_description (GtkWidget *widget, 
				      const gchar *name,
 ...
 */
void
xedit_utils_set_atk_relation (GtkWidget *obj1, 
			      GtkWidget *obj2, 
...
gboolean
xedit_utils_uri_exists (const gchar* text_uri)
{
	GFile *gfile;
	gboolean res;
	...
	xedit_debug_message (DEBUG_UTILS, "text_uri: %s", text_uri);
	...
	xedit_debug_message (DEBUG_UTILS, res ? "TRUE" : "FALSE");

	return res;
}

gchar *
xedit_utils_escape_search_text (const gchar* text)
{
	GString *str;
	gint length;
	const gchar *p;
...
gchar *
xedit_utils_unescape_search_text (const gchar *text)
{
	GString *str;
	gint length;
...
void 
xedit_warning (GtkWindow *parent, const gchar *format, ...)
{
	va_list         args;
	gchar          *str;
	GtkWidget      *dialog;
 ...
 */
gchar * 
xedit_utils_escape_underscores (const gchar* text,
				gssize       length)
{
	GString *str;
	const gchar *p;
...
static gchar *
xedit_utils_str_truncate (const gchar *string,
...
gchar *
xedit_utils_str_middle_truncate (const gchar *string,
				 guint        truncate_length)
{
	return xedit_utils_str_truncate (string, truncate_length, TRUE);
}

gchar *
xedit_utils_str_end_truncate (const gchar *string,
			      guint        truncate_length)
{
	return xedit_utils_str_truncate (string, truncate_length, FALSE);
}

gchar *
xedit_utils_make_valid_utf8 (const char *name)
{
	GString *string;
 ...
 */
gchar *
xedit_utils_uri_get_dirname (const gchar *uri)
{
	gchar *res;
	gchar *str;
	...
	res = xedit_utils_replace_home_dir_with_tilde (str);
...
/**
 * xedit_utils_location_get_dirname_for_display
 * @file: the location
 *
 ...
 */
gchar *
xedit_utils_location_get_dirname_for_display (GFile *location)
{
	gchar *uri;
	gchar *res;
	GMount *mount;
	...
	if (mount != NULL)
	{
		...
		if (path == NULL)
		{
			dirname = xedit_utils_uri_get_dirname (uri);
		}
		else
		{
			dirname = xedit_utils_uri_get_dirname (path);
		}
	...
	else
	{
		...
		res = xedit_utils_uri_get_dirname (uri);
	}
...
gchar *
xedit_utils_replace_home_dir_with_tilde (const gchar *uri)
{
	gchar *tmp;
	gchar *home;
 ...
 */
guint
xedit_utils_get_current_workspace (GdkScreen *screen)
{
#ifdef GDK_WINDOWING_X11
	GdkWindow *root_win;
	GdkDisplay *display;
 ...
 * it returns XEDIT_ALL_WORKSPACES if the window is sticky, or if
 * the window manager doesn support this function
 */
guint
xedit_utils_get_window_workspace (GtkWindow *gtkwindow)
{
#ifdef GDK_WINDOWING_X11
	GdkWindow *window;
	GdkDisplay *display;
	...
	guint *workspace;
	gint err, result;
	guint ret = XEDIT_ALL_WORKSPACES;
...
/**
 * xedit_utils_get_current_viewport:
 *
 ...
 */
void
xedit_utils_get_current_viewport (GdkScreen    *screen,
				  gint         *x,
				  gint         *y)
{
#ifdef GDK_WINDOWING_X11
...
gboolean
xedit_utils_is_valid_uri (const gchar *uri)
{
	const guchar *p;
 ...
 */
gboolean
xedit_utils_get_ui_objects (const gchar  *filename,
			    gchar       **root_objects,
...
gchar *
xedit_utils_make_canonical_uri_from_shell_arg (const gchar *str)
{	
	GFile *gfile;
	gchar *uri;
	 ...
	 * <paolo>: I will use xedit_utils_is_valid_uri ()
	 *
	 */
 ...
 */
gboolean
xedit_utils_file_has_parent (GFile *gfile)
{
	GFile *parent;
	gboolean ret;
...
xedit_utils_basename_for_display (gchar const *uri)
{
	...
	else if (xedit_utils_file_has_parent (gfile) || !xedit_utils_decode_uri (uri, NULL, NULL, &hn, NULL, NULL))
	{
	...
	else
	{
		...
		if  (hn != NULL)
			hn_utf8 = xedit_utils_make_valid_utf8 (hn);
		else
 ...
 */
gchar *
xedit_utils_uri_for_display (const gchar *uri)
{
	GFile *gfile;
	gchar *parse_name;
 ...
 */
gchar **
xedit_utils_drop_get_uris (GtkSelectionData *selection_data)
{
	gchar **uris;
	gint i;
	gint p = 0;
	...
	for (i = 0; uris[i] != NULL; i++)
	{
		...
		uri = xedit_utils_make_canonical_uri_from_shell_arg (uris[i]);
 ...
 */
gboolean
xedit_utils_decode_uri (const gchar  *uri,
			gchar       **scheme,
			gchar       **user,
			gchar       **host,
			gchar       **port,
