...
#define XEDIT_VIEW_SCROLL_MARGIN 0.02
#define XEDIT_VIEW_SEARCH_DIALOG_TIMEOUT (30*1000) /* 30 seconds */

#define MIN_SEARCH_COMPLETION_KEY_LEN	3

#define XEDIT_VIEW_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE ((object), XEDIT_TYPE_VIEW, XeditViewPrivate))
...
static void	xedit_view_dispose		(GObject          *object);
static void	xedit_view_finalize		(GObject          *object);
static gint     xedit_view_focus_out		(GtkWidget        *widget,
						 GdkEventFocus    *event);
static gboolean xedit_view_drag_motion		(GtkWidget        *widget,
						 GdkDragContext   *context,
						 gint              x,
						 gint              y,
						 guint             timestamp);
static void     xedit_view_drag_data_received   (GtkWidget        *widget,
						 GdkDragContext   *context,
						 gint              x,
						 gint              y,
						 ...
						 guint             info,
						 guint             timestamp);
static gboolean xedit_view_drag_drop		(GtkWidget        *widget,
	      					 GdkDragContext   *context,
	      					 gint              x,
	      					 gint              y,
	      					 guint             timestamp);
static gboolean	xedit_view_button_press_event	(GtkWidget        *widget,
						 GdkEventButton   *event);

static gboolean start_interactive_search	(XeditView        *view);
static gboolean start_interactive_goto_line	(XeditView        *view);
static gboolean reset_searched_text		(XeditView        *view);

static void	hide_search_window 		(XeditView        *view,
						 gboolean          cancel);

#if GTK_CHECK_VERSION (3, 0, 0)
static gboolean	xedit_view_draw		 	(GtkWidget        *widget,
						 cairo_t          *cr);
#else
static gint	xedit_view_expose	 	(GtkWidget        *widget,
						 GdkEventExpose   *event);
#endif
static void 	search_highlight_updated_cb	(XeditDocument    *doc,
						 GtkTextIter      *start,
						 GtkTextIter      *end,
						 XeditView        *view);

static void	xedit_view_delete_from_cursor 	(GtkTextView     *text_view,
						 GtkDeleteType    type,
						 gint             count);

#if GTK_CHECK_VERSION (3, 0, 0)
G_DEFINE_TYPE(XeditView, xedit_view, GTK_SOURCE_TYPE_VIEW)
#else
G_DEFINE_TYPE(XeditView, xedit_view, GTK_TYPE_SOURCE_VIEW)
#endif
...
typedef enum
{
	XEDIT_SEARCH_ENTRY_NORMAL,
	XEDIT_SEARCH_ENTRY_NOT_FOUND
} XeditSearchEntryState;

static void
document_read_only_notify_handler (XeditDocument *document, 
			           GParamSpec    *pspec,
				   XeditView     *view)
{
	xedit_debug (DEBUG_VIEW);

	gtk_text_view_set_editable (GTK_TEXT_VIEW (view), 
				    !xedit_document_get_readonly (document));
}

static void
xedit_view_class_init (XeditViewClass *klass)
{
	...
	object_class->dispose = xedit_view_dispose;
	object_class->finalize = xedit_view_finalize;

	widget_class->focus_out_event = xedit_view_focus_out;
#if GTK_CHECK_VERSION (3, 0, 0)
	widget_class->draw = xedit_view_draw;
#else
	widget_class->expose_event = xedit_view_expose;
#endif
	 ...
	 */
	widget_class->drag_motion = xedit_view_drag_motion;
	widget_class->drag_data_received = xedit_view_drag_data_received;
	widget_class->drag_drop = xedit_view_drag_drop;
	widget_class->button_press_event = xedit_view_button_press_event;
	...
	text_view_class->delete_from_cursor = xedit_view_delete_from_cursor;
	
	view_signals[START_INTERACTIVE_SEARCH] =
    		g_signal_new ("start_interactive_search",
		  	      G_TYPE_FROM_CLASS (object_class),
		  	      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
		  	      G_STRUCT_OFFSET (XeditViewClass, start_interactive_search),
			      NULL, NULL,
	...
	view_signals[START_INTERACTIVE_GOTO_LINE] =
    		g_signal_new ("start_interactive_goto_line",
		  	      G_TYPE_FROM_CLASS (object_class),
		  	      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
		  	      G_STRUCT_OFFSET (XeditViewClass, start_interactive_goto_line),
			      NULL, NULL,
			      xedit_marshal_BOOLEAN__NONE,
			      G_TYPE_BOOLEAN, 0);
	...
	view_signals[RESET_SEARCHED_TEXT] =
    		g_signal_new ("reset_searched_text",
		  	      G_TYPE_FROM_CLASS (object_class),
		  	      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
		  	      G_STRUCT_OFFSET (XeditViewClass, reset_searched_text),
			      NULL, NULL,
			      xedit_marshal_BOOLEAN__NONE,
			      G_TYPE_BOOLEAN, 0);		

	/* A new signal DROP_URIS has been added to allow plugins to intercept
	 * the default dnd behaviour of 'text/uri-list'. XeditView now handles
	 ...
	 * will not prevent xedit from loading the uri
	 */
	view_signals[DROP_URIS] =
    		g_signal_new ("drop_uris",
		  	      G_TYPE_FROM_CLASS (object_class),
		  	      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
		  	      G_STRUCT_OFFSET (XeditViewClass, drop_uris),
			      NULL, NULL,
			      g_cclosure_marshal_VOID__BOXED,
			      G_TYPE_NONE, 1, G_TYPE_STRV);
			      
	g_type_class_add_private (klass, sizeof (XeditViewPrivate));
...
static void
current_buffer_removed (XeditView *view)
{
...
static void
on_notify_buffer_cb (XeditView  *view,
		     GParamSpec *arg1,
		     gpointer    userdata)
{
	...
	if (buffer == NULL || !XEDIT_IS_DOCUMENT (buffer))
		return;
	...
	gtk_text_view_set_editable (GTK_TEXT_VIEW (view), 
				    !xedit_document_get_readonly (XEDIT_DOCUMENT (buffer)));
...
static void 
xedit_view_init (XeditView *view)
{
	GtkTargetList *tl;
	
	xedit_debug (DEBUG_VIEW);
	
	view->priv = XEDIT_VIEW_GET_PRIVATE (view);
	...
	if (!xedit_prefs_manager_get_use_default_font ())
	{
		...
		editor_font = xedit_prefs_manager_get_editor_font ();

		xedit_view_set_font (view, FALSE, editor_font);

		g_free (editor_font);
	}
	else
	{
		xedit_view_set_font (view, TRUE, NULL);
	}

	g_object_set (G_OBJECT (view), 
		      "wrap_mode", xedit_prefs_manager_get_wrap_mode (),
		      "show_line_numbers", xedit_prefs_manager_get_display_line_numbers (),
		      "auto_indent", xedit_prefs_manager_get_auto_indent (),
		      "tab_width", xedit_prefs_manager_get_tabs_size (),
		      "insert_spaces_instead_of_tabs", xedit_prefs_manager_get_insert_spaces (),
		      "show_right_margin", xedit_prefs_manager_get_display_right_margin (), 
		      "right_margin_position", xedit_prefs_manager_get_right_margin_position (),
		      "highlight_current_line", xedit_prefs_manager_get_highlight_current_line (),
		      "smart_home_end", xedit_prefs_manager_get_smart_home_end (),
		      "indent_on_tab", TRUE,
		      NULL);
...
static void
xedit_view_dispose (GObject *object)
{
	XeditView *view;

	view = XEDIT_VIEW (object);
	...
	(* G_OBJECT_CLASS (xedit_view_parent_class)->dispose) (object);
}

static void
xedit_view_finalize (GObject *object)
{
	XeditView *view;
	...
	(* G_OBJECT_CLASS (xedit_view_parent_class)->finalize) (object);
}

static gint
xedit_view_focus_out (GtkWidget *widget, GdkEventFocus *event)
{
	XeditView *view = XEDIT_VIEW (widget);
	...
	(* GTK_WIDGET_CLASS (xedit_view_parent_class)->focus_out_event) (widget, event);
 ...
 * @doc: a #XeditDocument
 * 
 * Creates a new #XeditView object displaying the @doc document. 
 * @doc cannot be %NULL.
 *
 * Return value: a new #XeditView
 **/
GtkWidget *
xedit_view_new (XeditDocument *doc)
{
	GtkWidget *view;

	xedit_debug_message (DEBUG_VIEW, "START");

	g_return_val_if_fail (XEDIT_IS_DOCUMENT (doc), NULL);

	view = GTK_WIDGET (g_object_new (XEDIT_TYPE_VIEW, "buffer", doc, NULL));

	xedit_debug_message (DEBUG_VIEW, "END: %d", G_OBJECT (view)->ref_count);
...
void
xedit_view_cut_clipboard (XeditView *view)
{
	GtkTextBuffer *buffer;
	...
	/* FIXME: what is default editability of a buffer? */
  	gtk_text_buffer_cut_clipboard (buffer,
				       ...
				       !xedit_document_get_readonly (
				       		XEDIT_DOCUMENT (buffer)));
  	
	gtk_text_view_scroll_to_mark (GTK_TEXT_VIEW (view),
				      gtk_text_buffer_get_insert (buffer),
				      XEDIT_VIEW_SCROLL_MARGIN,
				      FALSE,
				      0.0,
				      0.0);
}
...
void
xedit_view_copy_clipboard (XeditView *view)
{
	GtkTextBuffer *buffer;
	GtkClipboard *clipboard;
...
void
xedit_view_paste_clipboard (XeditView *view)
{
  	GtkTextBuffer *buffer;
	GtkClipboard *clipboard;
 ...
 **/
void
xedit_view_delete_selection (XeditView *view)
{
 ...
 **/
void
xedit_view_select_all (XeditView *view)
{
	GtkTextBuffer *buffer = NULL;
 ...
 **/
void
xedit_view_scroll_to_cursor (XeditView *view)
{
	GtkTextBuffer* buffer = NULL;
 ...
 **/
void
xedit_view_set_font (XeditView   *view, 
		     gboolean     def, 
...
set_entry_state (GtkWidget             *entry,
                 XeditSearchEntryState  state)
{
#if GTK_CHECK_VERSION (3, 0 ,0)
	GtkStyleContext *context = gtk_widget_get_style_context (GTK_WIDGET (entry));
#endif

	if (state == XEDIT_SEARCH_ENTRY_NOT_FOUND)
	{
#if GTK_CHECK_VERSION (3, 0 ,0)
...
static void
hide_search_window (XeditView *view, gboolean cancel)
{
...
static gboolean
search_entry_flush_timeout (XeditView *view)
{
#if !GTK_CHECK_VERSION (3, 0, 0)
	GDK_THREADS_ENTER ();
#endif
...
static void
update_search_window_position (XeditView *view)
{
	gint x, y;
	gint view_x, view_y;
	      ...
	      gboolean   search_backward)
{
	...
	if (view->priv->typeselect_flush_timeout != 0)
	{
		...
		view->priv->typeselect_flush_timeout =
			g_timeout_add (XEDIT_VIEW_SEARCH_DIALOG_TIMEOUT,
			       ...
			       XeditView   *view)
{
	...
	if (event->keyval == GDK_KEY_Escape)
	{
		if (view->priv->search_mode == SEARCH)
		{
			...
			doc = XEDIT_DOCUMENT (gtk_text_view_get_buffer (GTK_TEXT_VIEW (view)));
			xedit_document_set_search_text (doc, 
...
match_entire_word_menu_item_toggled (GtkCheckMenuItem *checkmenuitem,
				     XeditView        *view)
{
	XEDIT_SEARCH_SET_ENTIRE_WORD (view->priv->search_flags,
...
match_case_menu_item_toggled (GtkCheckMenuItem *checkmenuitem,
			      XeditView        *view)
{
	XEDIT_SEARCH_SET_CASE_SENSITIVE (view->priv->search_flags,
					 gtk_check_menu_item_get_active (checkmenuitem));
}
...
parse_escapes_menu_item_toggled (GtkCheckMenuItem *checkmenuitem,
			         XeditView        *view)
{
	XEDIT_SEARCH_SET_PARSE_ESCAPES (view->priv->search_flags,
					gtk_check_menu_item_get_active (checkmenuitem));
}
			     ...
			     XeditView *view)
{
	...
	gtk_menu_shell_prepend (GTK_MENU_SHELL (menu), menu_item);
	gtk_check_menu_item_set_active (GTK_CHECK_MENU_ITEM (menu_item),
					XEDIT_SEARCH_IS_ENTIRE_WORD (view->priv->search_flags));
	gtk_widget_show (menu_item);
	...
	gtk_menu_shell_prepend (GTK_MENU_SHELL (menu), menu_item);
	gtk_check_menu_item_set_active (GTK_CHECK_MENU_ITEM (menu_item),
					XEDIT_SEARCH_IS_CASE_SENSITIVE (view->priv->search_flags));
	gtk_widget_show (menu_item);
	...
	gtk_menu_shell_prepend (GTK_MENU_SHELL (menu), menu_item);
	gtk_check_menu_item_set_active (GTK_CHECK_MENU_ITEM (menu_item),
					XEDIT_SEARCH_IS_PARSE_ESCAPES (view->priv->search_flags));
	gtk_widget_show (menu_item);
}
			  ...
			  XeditView   *view)
{
	...
	else
	{
		...
		xedit_debug_message (DEBUG_SEARCH, "Text: %s", text);
		...
		xedit_debug_message (DEBUG_SEARCH, "Escaped Text: %s", escaped_text);
...
static void
customize_for_search_mode (XeditView *view)
{
	if (view->priv->search_mode == SEARCH)
	{
		 ...
		 gpointer            data)
{
	...
	gboolean ret = FALSE;
	GtkTreeModel *model;
	XeditViewPrivate *priv = (XeditViewPrivate *)data;
	...
	if (XEDIT_SEARCH_IS_CASE_SENSITIVE (priv->search_flags))
	{		
...
static void
ensure_search_window (XeditView *view)
{  
	GtkWidget          *frame;
	GtkWidget          *vbox;
	GtkWidget          *toplevel;
...
static void
init_search_entry (XeditView *view)
{
	GtkTextBuffer *buffer;
	...
	else
	{
		...
		old_find_text = xedit_document_get_search_text (XEDIT_DOCUMENT (buffer), 
								&old_find_flags);
	     ...
	     XeditView *view)
{
	...
	if (view->priv->search_mode == SEARCH)
	{
		...
		search_text = xedit_document_get_search_text (doc, &search_flags);
	...
	else
	{
		if (*entry_text != '\0')
		{
			...
			moved = xedit_document_goto_line (doc, line);
			moved_offset = xedit_document_goto_line_offset (doc, line,
									line_offset);
...
static gboolean
start_interactive_search_real (XeditView *view)
{	
	GtkTextBuffer *buffer;
...
static gboolean
reset_searched_text (XeditView *view)
{		
	XeditDocument *doc;

	doc = XEDIT_DOCUMENT (gtk_text_view_get_buffer (GTK_TEXT_VIEW (view)));
	
	xedit_document_set_search_text (doc, "", XEDIT_SEARCH_DONT_SET_FLAGS);
	
	return TRUE;
}

static gboolean
start_interactive_search (XeditView *view)
{		
...
static gboolean 
start_interactive_goto_line (XeditView *view)
{
...
#if GTK_CHECK_VERSION (3, 0, 0)
static gboolean
xedit_view_draw (GtkWidget      *widget,
                 cairo_t        *cr)
#else
static gint
xedit_view_expose (GtkWidget      *widget,
                   GdkEventExpose *event)
#endif
{
	...
	doc = XEDIT_DOCUMENT (gtk_text_view_get_buffer (text_view));
#if GTK_CHECK_VERSION (3, 0, 0)
...
#endif
	    xedit_document_get_enable_search_highlighting (doc))
	{
		...
		_xedit_document_search_region (doc,
					       &iter1,
					       &iter2);
	}

#if GTK_CHECK_VERSION (3, 0, 0)
	return GTK_WIDGET_CLASS (xedit_view_parent_class)->draw (widget, cr);
#else
	return (* GTK_WIDGET_CLASS (xedit_view_parent_class)->expose_event)(widget, event);
#endif
}
...
static gboolean
xedit_view_drag_motion (GtkWidget      *widget,
			GdkDragContext *context,
			gint            x,
			gint            y,
			guint           timestamp)
{
	...
	result = GTK_WIDGET_CLASS (xedit_view_parent_class)->drag_motion (widget, context, x, y, timestamp);
...
static void
xedit_view_drag_data_received (GtkWidget        *widget,
		       	       GdkDragContext   *context,
			       gint              x,
			       gint              y,
			       GtkSelectionData *selection_data,
			       ...
			       guint             timestamp)
{
	...
	{
		GTK_WIDGET_CLASS (xedit_view_parent_class)->drag_data_received (widget, context, x, y, selection_data, info, timestamp);
	}
}

static gboolean
xedit_view_drag_drop (GtkWidget      *widget,
		      GdkDragContext *context,
		      gint            x,
		      gint            y,
		      guint           timestamp)
{
	...
	{
		/* Chain up */
		result = GTK_WIDGET_CLASS (xedit_view_parent_class)->drag_drop (widget, context, x, y, timestamp);
	}
			   ...
			   XeditView *view)
{
	...
	xedit_prefs_manager_set_display_line_numbers (show);
}
...
static gboolean
xedit_view_button_press_event (GtkWidget *widget, GdkEventButton *event)
{
	...
	return GTK_WIDGET_CLASS (xedit_view_parent_class)->button_press_event (widget, event);
}

static void 	
search_highlight_updated_cb (XeditDocument *doc,
			     GtkTextIter   *start,
			     GtkTextIter   *end,
			     XeditView     *view)
{
	...
	g_return_if_fail (xedit_document_get_enable_search_highlighting (
				XEDIT_DOCUMENT (gtk_text_view_get_buffer (text_view))));
...
static void
xedit_view_delete_from_cursor (GtkTextView   *text_view,
			       GtkDeleteType  type,
			       gint           count)
{
	...
	switch (type)
	{
		case GTK_DELETE_PARAGRAPHS:
			...
			break;
		default:
			GTK_TEXT_VIEW_CLASS (xedit_view_parent_class)->delete_from_cursor(text_view, type, count);
			break;
	}
}
